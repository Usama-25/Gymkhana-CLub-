...
{
    public partial class Weedout : System.Web.UI.Page
    {
        ...
        private short CurrentStaffID
        {
            get
            {
                ...
                return 1; // default fallback (admin/librarian)
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                // Optionally check query parameters if coming from catalogue
        ...
        private void BindBooksGrid(string term)
        {
            try
            {
                ...
                // Search books using existing DBHelper search method
                DataTable dt = GymkhanaLibrary.Weedout.DBHelper.SearchBooks(term, null, null, null, null, null, false, null, null, null, null, bookNo);
        ...
        private void SelectBook(int bookID)
        {
            try
            {
                ...
                lblBookTitle.Text = r["Title"].ToString();
                lblBookISBN.Text = r["ISBN13"].ToString();
                lblBookCategory.Text = r["CatName"].ToString();
                lblBookEdition.Text = r["Edition"] != DBNull.Value ? r["Edition"].ToString() : "N/A";
                lblBookClassNo.Text = r["ClassNo"] != DBNull.Value ? r["ClassNo"].ToString() : "N/A";
                
                ViewState["SelectedBookID"] = bookID;
                pnlManageWeeding.Visible = true;
                pnlWeedIndividual.Visible = false;
                pnlRestoreCopy.Visible = false;

                BindBookCopies(bookID);
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading book details: " + ex.Message, "alert-error");
            }
        }

        private void BindBookCopies(int bookID)
        {
            try
            {
                DataTable dt = DBHelper.GetBookCopiesForWeeding(bookID);
                gvCopies.DataSource = dt;
                gvCopies.DataBind();

                // Live update summary label counts
                int total = dt.Rows.Count;
                int available = 0;
                foreach (DataRow row in dt.Rows)
                {
                    if (Convert.ToInt32(row["CondID"]) != 7 && Convert.ToBoolean(row["IsAvailable"]))
                    {
                        available++;
                    }
                }
                lblTotalCopies.Text = total.ToString();
                lblAvailableCopies.Text = available.ToString();
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading copy details: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void gvCopies_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "WeedCopy")
            {
                ShowWeedPanel(copyID);
            }
            else if (e.CommandName == "RestoreCopy")
            {
                ShowRestorePanel(copyID);
            }
        }

        private void ShowWeedPanel(int copyID)
        {
            try
            {
                DataTable dt = DBHelper.GetTableData("SELECT Barcode FROM BookCopies WHERE CopyID = " + copyID);
                if (dt.Rows.Count > 0)
                {
                    hfWeedCopyID.Value = copyID.ToString();
                    lblWeedBarcode.Text = dt.Rows[0]["Barcode"].ToString();
                    txtWeedRemarks.Text = "";
                    
                    pnlWeedIndividual.Visible = true;
                    pnlRestoreCopy.Visible = false;
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading copy: " + ex.Message, "alert-error");
            }
        }

        private void ShowRestorePanel(int copyID)
        {
            try
            {
                DataTable dt = DBHelper.GetTableData("SELECT Barcode FROM BookCopies WHERE CopyID = " + copyID);
                if (dt.Rows.Count > 0)
                {
                    hfRestoreCopyID.Value = copyID.ToString();
                    lblRestoreBarcode.Text = dt.Rows[0]["Barcode"].ToString();
                    txtRestoreRemarks.Text = "";
                    ddlRestoreCondition.SelectedIndex = 1; // Good
                    
                    // Bind halls to trigger location allocation cascading dropdowns
                    BindHalls();

                    pnlWeedIndividual.Visible = false;
                    pnlRestoreCopy.Visible = true;
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error preparing restoration panel: " + ex.Message, "alert-error");
            }
        }

        protected void btnConfirmWeed_Click(object sender, EventArgs e)
        {
            int copyID = Convert.ToInt32(hfWeedCopyID.Value);
            string remarks = txtWeedRemarks.Text.Trim();
            if (string.IsNullOrEmpty(remarks))
            {
                ShowAlert("Weeding remarks are required to weed out a copy.", "alert-error");
                return;
            }
            ...
            try
            {
                ...
                {
                    ShowAlert(res, "alert-success");
                    pnlWeedIndividual.Visible = false;
                    
                    int bookID = Convert.ToInt32(ViewState["SelectedBookID"]);
                    BindBookCopies(bookID);
                }
                else
                {
                    ShowAlert(res, "alert-error");
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error weeding out copy: " + ex.Message, "alert-error");
            }
        }

        protected void btnBulkWeed_Click(object sender, EventArgs e)
        {
            int bookID = Convert.ToInt32(ViewState["SelectedBookID"]);
            string remarks = txtBulkRemarks.Text.Trim();
            if (string.IsNullOrEmpty(remarks))
            {
                ShowAlert("Bulk weeding remarks are required to weed out all copies.", "alert-error");
                return;
            }
            ...
            {
                ShowAlert("Error weed out book copies: " + ex.Message, "alert-error");
            }
        }

        protected void btnConfirmRestore_Click(object sender, EventArgs e)
        {
            int copyID = Convert.ToInt32(hfRestoreCopyID.Value);
            byte condID = Convert.ToByte(ddlRestoreCondition.SelectedValue);
            string remarks = txtRestoreRemarks.Text.Trim();
            
            if (ddlHall.SelectedValue == "0" || ddlAisle.SelectedValue == "0" || ddlShelfUnit.SelectedValue == "0" || ddlRack.SelectedValue == "0" || ddlSlot.SelectedValue == "0")
            {
                ShowAlert("Please complete the shelf location allocation (Hall, Aisle, Shelf, Rack, Slot).", "alert-error");
                return;
            }

            short rackID = Convert.ToInt16(ddlRack.SelectedValue);
            byte slotNo = Convert.ToByte(ddlSlot.SelectedValue);

            if (string.IsNullOrEmpty(remarks))
            {
                ShowAlert("Remarks are required to restore a copy.", "alert-error");
                return;
            }

            try
            {
                string res = DBHelper.RestoreCopy(copyID, condID, rackID, slotNo, remarks, CurrentStaffID);
                if (res.StartsWith("SUCCESS"))
                {
                    ShowAlert(res, "alert-success");
                    pnlRestoreCopy.Visible = false;
            ...
            {
                ShowAlert("Error restoring copy: " + ex.Message, "alert-error");
            }
        }
        ...
        private void BindHalls()
        {
            ...
            ddlAisle.Items.Insert(0, new ListItem("- Select Aisle -", "0"));
            ddlAisle.Enabled = false;

            ddlShelfUnit.Items.Clear();
            ddlShelfUnit.Items.Insert(0, new ListItem("- Select Shelf Unit -", "0"));
        ...
        protected void ddlHall_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                short hallID = Convert.ToInt16(ddlHall.SelectedValue);
                DataTable dt = DBHelper.GetAisles(hallID);
                
                ddlAisle.DataSource = dt;
                ddlAisle.DataTextField = "AisleDisplay";
                ddlAisle.DataValueField = "AisleID";
                ddlAisle.DataBind();
                ddlAisle.Items.Insert(0, new ListItem("- Select Aisle -", "0"));
                ddlAisle.Enabled = true;
            }
            else
            {
                ddlAisle.Items.Clear();
        ...
        {
            if (ddlAisle.SelectedValue != "0")
            {
                int aisleID = Convert.ToInt32(ddlAisle.SelectedValue);
                DataTable dt = DBHelper.GetShelfUnits(aisleID);

                ddlShelfUnit.DataSource = dt;
                ddlShelfUnit.DataTextField = "ShelfUnitCode";
                ddlShelfUnit.DataValueField = "ShelfUnitID";
                ddlShelfUnit.DataBind();
                ddlShelfUnit.Items.Insert(0, new ListItem("- Select Shelf Unit -", "0"));
                ddlShelfUnit.Enabled = true;
            }
            else
            {
                ddlShelfUnit.Items.Clear();
        ...
        protected void ddlShelfUnit_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlShelfUnit.SelectedValue != "0")
            {
                int unitID = Convert.ToInt32(ddlShelfUnit.SelectedValue);
                DataTable dt = DBHelper.GetRacks(unitID);

                ddlRack.DataSource = dt;
                ddlRack.DataTextField = "RackDisplay";
        ...
        private void BindSlots(short rackID)
        {
            try
            {
                DataTable rackData = DBHelper.GetTableData("SELECT TotalSlots FROM Racks WHERE RackID = " + rackID);
                if (rackData.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in slotsDt.Rows)
                    {
                        ...
                        if (!isOccupied)
                        {
                            string num = row["SlotNumber"].ToString();
                            ddlSlot.Items.Add(new ListItem("Slot " + num, num));
                        }
                    }
            ...
            {
                ShowAlert("Error loading slots: " + ex.Message, "alert-error");
            }
        }

...
public partial class Pages_Members_MemberLedger : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            // Check if MemberID or MemberNo is passed in URL
            if (!string.IsNullOrEmpty(Request.QueryString["MemberID"]))
            {
                int memberID;
                if (int.TryParse(Request.QueryString["MemberID"], out memberID))
                {
                    hfSelectedMemberID.Value = memberID.ToString();
                    DataRow dr = DBHelper.GetMemberDetails(memberID);
                    if (dr != null)
                    {
                        txtMemberNo.Text = dr["MemberNo"].ToString();
                        txtMemberName.Text = dr["MemberName"].ToString();
                        BindMemberLedger(memberID);
                    }
                }
            }
            else if (!string.IsNullOrEmpty(Request.QueryString["MemberNo"]))
            {
                string memberNoStr = Request.QueryString["MemberNo"].Trim();
                DataTable dtM = DBHelper.GetMembers(memberNoStr);
                if (dtM != null && dtM.Rows.Count > 0)
                {
                    int memberID = Convert.ToInt32(dtM.Rows[0]["MemberID"]);
                    hfSelectedMemberID.Value = memberID.ToString();
                    txtMemberNo.Text = dtM.Rows[0]["MembershipNo"].ToString();
                    txtMemberName.Text = dtM.Rows[0]["FullName"].ToString();
                    BindMemberLedger(memberID);
                }
            }
        }
    ...
    private void BindYearDropdown()
    {
        ...
        // Bind years from 2024 to current year + 1
    ...
    {
        string memberNoText = txtMemberNo.Text.Trim();
        string memberNameText = txtMemberName.Text.Trim();

        if (string.IsNullOrEmpty(memberNoText) && string.IsNullOrEmpty(memberNameText))
        {
            ShowAlert("Please enter a member number or name to search.", false);
            return;
        }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(memberNoText))
            {
                searchKey = memberNoText;
            }
            else if (!string.IsNullOrEmpty(memberNameText))
            {
                searchKey = memberNameText;
            }

            // If autocomplete full text is selected, strip to key
            ...
            foreach (DataRow row in dt.Rows)
            {
                if (row["MembershipNo"].ToString().Equals(searchKey, StringComparison.OrdinalIgnoreCase) ||
                    row["FullName"].ToString().Equals(searchKey, StringComparison.OrdinalIgnoreCase) ||
                    row["MemberDisplay"].ToString().Equals(searchKey, StringComparison.OrdinalIgnoreCase))
                {
                    memberID = Convert.ToInt32(row["MemberID"]);
                    // Update search box display inputs
                    txtMemberNo.Text = row["MembershipNo"].ToString();
                    txtMemberName.Text = row["FullName"].ToString();
                    break;
                }
            }
            ...
            if (memberID == null && dt.Rows.Count > 0)
            {
                ...
                txtMemberNo.Text = dt.Rows[0]["MembershipNo"].ToString();
            ...
            {
                hfSelectedMemberID.Value = memberID.ToString();
                BindMemberLedger(memberID.Value);
            }
            else
            {
                hfSelectedMemberID.Value = "";
                pnlLedgerContent.Visible = false;
                pnlEmptyState.Visible = true;
                ShowAlert("No library member matching \"" + searchKey + "\" was found.", false);
            }
        }
        catch (Exception ex)
        {
            ShowAlert("Error searching for member: " + ex.Message, false);
        }
    }

    private void BindMemberLedger(int memberID)
    {
        try
        {
            ...
            if (dr == null)
            {
                ShowAlert("Failed to load member profile details.", false);
                return;
            }

            // Basic Information
            lblMembershipNo.Text = dr["MemberNo"] == DBNull.Value ? "-" : dr["MemberNo"].ToString();
            lblFullName.Text = dr["MemberName"] == DBNull.Value ? "-" : dr["MemberName"].ToString();
            lblMemberType.Text = dr["MemberType"] == DBNull.Value ? "-" : dr["MemberType"].ToString();

            lblCNIC.Text = dr["NIC"] == DBNull.Value || string.IsNullOrWhiteSpace(dr["NIC"].ToString())
                ? "-"
                : dr["NIC"].ToString();

            lblPhone.Text = dr["Phone"] == DBNull.Value || string.IsNullOrWhiteSpace(dr["Phone"].ToString())
                ? "-"
                : dr["Phone"].ToString();

            lblEmail.Text = dr["ResidentialEmail"] == DBNull.Value || string.IsNullOrWhiteSpace(dr["ResidentialEmail"].ToString())
                ? "-"
                : dr["ResidentialEmail"].ToString();

            lblJoinDate.Text = dr["MemberSince"] == DBNull.Value
                ? "-"
                : Convert.ToDateTime(dr["MemberSince"]).ToString("dd MMM yyyy");

            // Status
            bool isActive = dr["IsActive"] != DBNull.Value &&
                            Convert.ToBoolean(dr["IsActive"]);

            if (isActive)
            {
                lblStatusBadge.Text =
                    "<span style='background-color:#d1fae5;color:#065f46;padding:4px 10px;border-radius:30px;font-size:11px;font-weight:700;'>Active</span>";
            }
            else
            {
                lblStatusBadge.Text =
                    "<span style='background-color:#fee2e2;color:#991b1b;padding:4px 10px;border-radius:30px;font-size:11px;font-weight:700;'>Inactive</span>";
            }

            // Summary Badges
            lblTotalLoans.Text = dr["TotalLoans"] == DBNull.Value
                ? "0"
                : Convert.ToInt32(dr["TotalLoans"]).ToString();

            lblActiveLoans.Text = dr["ActiveLoans"] == DBNull.Value
                ? "0"
                : Convert.ToInt32(dr["ActiveLoans"]).ToString();

            lblTotalFines.Text = dr["TotalFines"] == DBNull.Value
                ? "0.00"
                : Convert.ToDecimal(dr["TotalFines"]).ToString("N2");

            lblOutstandingBalance.Text = dr["OutstandingFines"] == DBNull.Value
                ? "0.00"
                : Convert.ToDecimal(dr["OutstandingFines"]).ToString("N2");
            ...
            if (!string.IsNullOrWhiteSpace(txtStartDate.Text) &&
                DateTime.TryParse(txtStartDate.Text, out tempDate))
            {
                startDate = tempDate;
            }
            ...
            // Ledger Data
            DataTable dtLedger = DBHelper.GetMemberLedger(
                memberID,
                startDate,
                endDate,
                month,
            ...
            gvLedger.DataSource = dtLedger;
            gvLedger.DataBind();

            pnlLedgerContent.Visible = true;
            pnlEmptyState.Visible = false;
        }
        catch (Exception ex)
        {
            ShowAlert(
                "Error rendering member ledger statement:<br/>" +
                ex.Message,
                false
            );
        }
    ...
    // CSS Badge Style Helper Methods
    protected string GetTxnTypeBadgeStyle(string type)
    {
        if (type == "Book Loan") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #e0f2fe; color: #075985;";
        if (type == "Library Fine") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #fee2e2; color: #991b1b;";
        if (type == "Facility Booking") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #fef3c7; color: #92400e;";
        return "";
    }

    protected string GetStatusBadgeStyle(string status)
    {
        if (status == "Paid" || status == "Returned") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #d1fae5; color: #065f46;";
        if (status == "Unpaid" || status == "Overdue") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #fee2e2; color: #991b1b;";
        if (status == "Issued") return "display: inline-block; padding: 4px 10px; border-radius: 30px; font-size: 11px; font-weight: 700; background-color: #fef3c7; color: #92400e;";
        return "";
    }
    ...
    {
        if (gvLedger.Rows.Count == 0)
        {
            ShowAlert("There is no ledger data to export. Generate a member ledger statement first.", false);
            return;
        }
        ...
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=LGC_MemberLedger_" + lblMembershipNo.Text + "_" + DateTime.Now.ToString("yyyyMMdd") + ".xls");
            Response.Charset = "";
        ...
        {
            ShowAlert("Error exporting member ledger to Excel: " + ex.Message, false);
        }
    }

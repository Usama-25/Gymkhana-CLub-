...
public partial class Pages_Admin_Dashboard : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ViewState["CurrentGridMode"] = "OVERDUES";
            LoadKPIs();
            LoadGridData();
            LoadRackOccupancy();
        }
    ...
    {
        DataTable dt = DBHelper.GetDashboardStats();
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            int totalCopies = Convert.ToInt32(row["TotalCopies"]);
            int availCopies = Convert.ToInt32(row["AvailableCopies"]);
            litAvailableCopies.Text = availCopies.ToString();
            
            litActiveMembers.Text = row["ActiveMembers"].ToString();
            litBooksOut.Text = row["LoansActive"].ToString();
            litOverdue.Text = row["Overdue"].ToString();

            decimal fines = Convert.ToDecimal(row["UnpaidFinesPKR"]);
            litFines.Text = fines.ToString("N2");
            
            // Expected returns today count
            litTodayReturns.Text = row["TodayReturns"].ToString();
            
            // Reminders Queue counts
            litRemindersGentle.Text = row["RemindersGentle"].ToString();
            litRemindersHarsh.Text = row["RemindersHarsh"].ToString();
            litRemindersFinal.Text = row["RemindersFinal"].ToString();
        }
    }

    private void LoadGridData()
    {
        string mode = ViewState["CurrentGridMode"] as string ?? "OVERDUES";
        if (mode == "OVERDUES")
        {
            litGridTitle.Text = "Overdue Loans and Warnings";
            spnBadge.Attributes["style"] = "background-color: #fee2e2; color: #ef4444; font-weight: 700; padding: 2px 6px; border-radius: 4px; font-size: 10px; text-transform: uppercase;";
            
            DataTable dt = DBHelper.GetOverdueReport();
            litOverdueCountBadge.Text = dt != null ? dt.Rows.Count.ToString() + " Overdue" : "0 Overdue";
            
            if (dt != null && dt.Rows.Count > 0)
            {
                rptOverdues.DataSource = dt;
                rptOverdues.DataBind();
                rptOverdues.Visible = true;
                pnlNoOverdues.Visible = false;
            }
            else
            {
                rptOverdues.Visible = false;
                pnlNoOverdues.Visible = true;
                litNoRecordsMsg.Text = "Excellent! No overdue loans checked out in the system currently.";
            }
        }
        else if (mode == "RETURNS")
        {
            litGridTitle.Text = "Expected Book Returns Today";
            spnBadge.Attributes["style"] = "background-color: #e0f2fe; color: #0369a1; font-weight: 700; padding: 2px 6px; border-radius: 4px; font-size: 10px; text-transform: uppercase;";
            
            DataTable dt = DBHelper.GetTodayReturnsReport();
            litOverdueCountBadge.Text = dt != null ? dt.Rows.Count.ToString() + " Today" : "0 Today";
            ...
            else
            {
                rptOverdues.Visible = false;
                pnlNoOverdues.Visible = true;
                litNoRecordsMsg.Text = "No books are expected to be returned today.";
            }
        }
    }

    protected void lnkOverduesCard_Click(object sender, EventArgs e)
    {
        ViewState["CurrentGridMode"] = "OVERDUES";
        LoadGridData();
    }

    protected void lnkTodayReturnsCard_Click(object sender, EventArgs e)
    {
        ViewState["CurrentGridMode"] = "RETURNS";
        LoadGridData();
    }

    private void LoadRackOccupancy()
    {
        DataTable dt = DBHelper.GetRackOccupancy(null);
        rptRackOccupancy.DataSource = dt;
        rptRackOccupancy.DataBind();
    }

    // --------------------------------------------------------------
    //  Mark-up Helpers (Visual bindings)
    // --------------------------------------------------------------
    protected string GetOccupancyColor(double pct)
    {
        if (pct >= 90)
        {
            return "#ef4444"; // Red (Warning: Rack is full)
        }
        else if (pct >= 70)
        {
            return "#f59e0b"; // Amber (Moderate fill)
        }
        else
        {
            return "#10b981"; // Emerald Green (Plenty of space)
        }
    }

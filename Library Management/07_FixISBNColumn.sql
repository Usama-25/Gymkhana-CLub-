-- =============================================================================
--  FIX: Widen ISBN13 column and Barcode for custom alphanumeric ISBN format
--  Format: AUTHOR-PUBLISHER-EDITION-LANGUAGE-SEQ  e.g. HAM-PEN-3RD-EN-001
...
-- Drop unique index/constraint on ISBN13 if it exists
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'UQ__Books__3C19D1D8' OR object_id = OBJECT_ID('Books') AND is_unique = 1)
BEGIN
...
-- Widen ISBN13 from CHAR(13) to VARCHAR(50) for custom alphanumeric format
ALTER TABLE Books ALTER COLUMN ISBN13 VARCHAR(50) NOT NULL;
GO

-- Re-add UNIQUE constraint on ISBN13
ALTER TABLE Books ADD CONSTRAINT UQ_Books_ISBN13 UNIQUE (ISBN13);
GO

-- Add conditional CHECK: only validate EAN-13 check digit if value is exactly 13 digits
ALTER TABLE Books ADD CONSTRAINT CK_ISBN13_Valid CHECK (
    -- If it's a standard 13-digit numeric ISBN, validate check digit
    -- Otherwise (custom alphanumeric format), allow it through
    LEN(ISBN13) > 0
    AND (
        -- Custom alphanumeric format: just ensure non-empty (already covered above)
        ISBN13 LIKE '%[^0-9]%'
        -- OR standard 13-digit numeric: must pass EAN-13 validation
        OR (LEN(ISBN13) = 13 AND ISBN13 NOT LIKE '%[^0-9]%' AND dbo.fn_ISBN13Valid(ISBN13) = 1)
        -- OR numeric but not 13 digits: allow (e.g. shorter numeric codes)
        OR (ISBN13 NOT LIKE '%[^0-9]%' AND LEN(ISBN13) <> 13)
    )
);
GO

-- Widen BookCopies.Barcode from CHAR(17) to VARCHAR(60)
-- First drop the unique index/constraint
IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id = OBJECT_ID('BookCopies') AND name LIKE '%Barcode%')
BEGIN
    ...
    JOIN sys.index_columns ic ON i.object_id = ic.object_id AND i.index_id = ic.index_id
    JOIN sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
    WHERE i.object_id = OBJECT_ID('BookCopies') AND c.name = 'Barcode' AND i.is_unique = 1;
...
-- =============================================================================
--  Step 3: Recreate vw_Books view (handle both standard and custom ISBNs)
-- =============================================================================
CREATE VIEW vw_Books WITH SCHEMABINDING AS
SELECT
    b.BookID,
    b.ISBN13,
    -- Format only if it's a standard 13-digit numeric ISBN, otherwise show as-is
    CASE 
...
-- =============================================================================
--  Step 4: Update sp_AddCopy barcode generation for wider ISBN
-- =============================================================================
...
-- =============================================================================
--  Step 5: Update sp_GetBookDetail to handle custom ISBN format
-- =============================================================================
...
PRINT '=== ISBN13 column widened and constraints updated for custom format ===';
GO

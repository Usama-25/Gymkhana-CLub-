...
public partial class Pages_Books_BookList : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            // Display redirect success messages (e.g. from AddEditBook.aspx)
            if (Session["BookSuccessMessage"] != null)
            {
                pnlSuccess.Visible = true;
                litSuccessMsg.Text = Session["BookSuccessMessage"].ToString();
                Session["BookSuccessMessage"] = null; // clear
            }

            // Restore filter state from QueryString to support stateless bookmarking/navigation
            if (Request.QueryString["q"] != null) txtSearch.Text = Request.QueryString["q"];
            if (Request.QueryString["cat"] != null) ddlCategory.SelectedValue = Request.QueryString["cat"];
            if (Request.QueryString["lang"] != null) ddlLanguage.SelectedValue = Request.QueryString["lang"];
            if (Request.QueryString["pub"] != null) txtPublisher.Text = Request.QueryString["pub"];
            if (Request.QueryString["ddc"] != null) txtDDC.Text = Request.QueryString["ddc"];
            if (Request.QueryString["bookno"] != null) txtBookNo.Text = Request.QueryString["bookno"];
            if (Request.QueryString["avail"] == "1") chkAvailOnly.Checked = true;
    ...
    {
        // 1. Categories
        ddlCategory.DataSource = DBHelper.GetCategories();
        ddlCategory.DataTextField = "CatName";
        ddlCategory.DataValueField = "CatID";
        ddlCategory.DataBind();
        ddlCategory.Items.Insert(0, new ListItem("- All Subjects -", "0"));

        // 2. Languages
        ddlLanguage.DataSource = DBHelper.GetLanguages();
        ddlLanguage.DataTextField = "LangName";
        ddlLanguage.DataValueField = "LangID";
        ddlLanguage.DataBind();
        ddlLanguage.Items.Insert(0, new ListItem("- All Languages -", "0"));

        // Publisher dropdown is replaced by an autocomplete TextBox dlPublishers
    }
    ...
    private string GetPageUrl(int pageNum)
    {
        ...
        if (!string.IsNullOrEmpty(txtSearch.Text)) queryParams["q"] = txtSearch.Text.Trim();
        if (ddlCategory.SelectedValue != "0") queryParams["cat"] = ddlCategory.SelectedValue;
        if (ddlLanguage.SelectedValue != "0") queryParams["lang"] = ddlLanguage.SelectedValue;
        if (!string.IsNullOrEmpty(txtPublisher.Text)) queryParams["pub"] = txtPublisher.Text.Trim();
        if (!string.IsNullOrEmpty(txtDDC.Text)) queryParams["ddc"] = txtDDC.Text.Trim();
        if (!string.IsNullOrEmpty(txtBookNo.Text)) queryParams["bookno"] = txtBookNo.Text.Trim();
        if (chkAvailOnly.Checked) queryParams["avail"] = "1";
        
        queryParams["page"] = pageNum.ToString();

        return "BookList.aspx?" + queryParams.ToString();
    }

    private void PerformSearch()
    {
        string term = !string.IsNullOrEmpty(txtSearch.Text) ? txtSearch.Text.Trim() : null;
        short? catID = ddlCategory.SelectedValue != "0" ? (short?)Convert.ToInt16(ddlCategory.SelectedValue) : null;
        byte? langID = ddlLanguage.SelectedValue != "0" ? (byte?)Convert.ToByte(ddlLanguage.SelectedValue) : null;
        bool availOnly = chkAvailOnly.Checked;
        string ddc = !string.IsNullOrEmpty(txtDDC.Text) ? txtDDC.Text.Trim() : null;
        int? bookNo = null;
        if (!string.IsNullOrEmpty(txtBookNo.Text))
        {
            int parsed;
            if (int.TryParse(txtBookNo.Text.Trim(), out parsed))
            {
                bookNo = parsed;
            }
            else
            {
                bookNo = -1; // invalid number, return 0 results
            }
        }

        string selectedPublisher = txtPublisher.Text.Trim();
        short? pubID = null;
        if (!string.IsNullOrEmpty(selectedPublisher))
        {
            DataTable dtPub = DBHelper.GetTableData("SELECT PubID FROM Publishers WHERE PubName = N'" + selectedPublisher.Replace("'", "''") + "' AND IsActive = 1");
            if (dtPub.Rows.Count > 0)
            {
                pubID = Convert.ToInt16(dtPub.Rows[0]["PubID"]);
            }
            else
            {
                pubID = -1; // If a publisher filter is entered but doesn't exist, return 0 results
            }
        }

        // Requirement 3: Searching with a single word should display all relevant records on one page.
        bool isSingleWordSearch = false;
        if (!string.IsNullOrEmpty(term))
        {
            string[] words = term.Split(new char[] { ' ', '\t', '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            if (words.Length == 1)
            {
                isSingleWordSearch = true;
            }
        }

        int currentPage = 1;
        if (!isSingleWordSearch && Request.QueryString["page"] != null)
        {
            int.TryParse(Request.QueryString["page"], out currentPage);
        }
        if (currentPage < 1) currentPage = 1;

        int pageSize = isSingleWordSearch ? 100000 : 20;

        DataTable dt = Pages_Books_BookList.DBHelper.SearchBooks(term, catID, langID, pubID, null, null, availOnly, null, isSingleWordSearch ? (int?)null : currentPage, pageSize, ddc, bookNo);
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            pds.AllowCustomPaging = !isSingleWordSearch;
            pds.AllowPaging = !isSingleWordSearch;
            if (!isSingleWordSearch)
            {
                pds.VirtualCount = totalRows;
                pds.PageSize = 20;
            ...
            // Bind Pager Controls (hidden for single-word searches)
            if (!isSingleWordSearch && pds.PageCount > 1)
            {
                pnlPager.Visible = true;
                
                lnkPrev.NavigateUrl = GetPageUrl(currentPage - 1);
                lnkPrev.Enabled = !pds.IsFirstPage;
                lnkPrev.Style["opacity"] = pds.IsFirstPage ? "0.5" : "1";
                lnkPrev.Style["pointer-events"] = pds.IsFirstPage ? "none" : "auto";

                lnkNext.NavigateUrl = GetPageUrl(currentPage + 1);
                lnkNext.Enabled = !pds.IsLastPage;
                lnkNext.Style["opacity"] = pds.IsLastPage ? "0.5" : "1";
                lnkNext.Style["pointer-events"] = pds.IsLastPage ? "none" : "auto";

                var pagerTable = new DataTable();
                pagerTable.Columns.Add("Text", typeof(string));
                pagerTable.Columns.Add("Url", typeof(string));
                pagerTable.Columns.Add("IsActive", typeof(bool));

                int startPage = Math.Max(1, currentPage - 2);
                int endPage = Math.Min(pds.PageCount, startPage + 4);
                ...
                {
                    var r = pagerTable.NewRow();
                    r["Text"] = i.ToString();
                    r["Url"] = GetPageUrl(i);
                    r["IsActive"] = (i == currentPage);
                    pagerTable.Rows.Add(r);
                }

                rptPager.DataSource = pagerTable;
                rptPager.DataBind();
            }
            else
            {
        ...
        {
            if (currentPage > 1 && !isSingleWordSearch)
            {
    ...
    protected void rptBooks_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Issue")
        {
            ...
            Response.Redirect("IssueReturn.aspx?BookID=" + bookID);
        }
    }

    // --------------------------------------------------------------
    //  Mark-up Helpers (Visual bindings)
    // --------------------------------------------------------------
    protected string GetBookNoDisplay(object bookNoObj, object bookIDObj)
    {
        if (bookNoObj != null && bookNoObj != DBNull.Value && !string.IsNullOrEmpty(bookNoObj.ToString()))
        {
            return bookNoObj.ToString().ToUpper();
        }

        if (bookIDObj != null && bookIDObj != DBNull.Value)
        {
            int bookID = Convert.ToInt32(bookIDObj);
            DataTable dtCopy = DBHelper.GetTableData("SELECT TOP 1 BookNo FROM BookCopies WHERE BookID = " + bookID + " AND BookNo IS NOT NULL ORDER BY CopyID");
            if (dtCopy != null && dtCopy.Rows.Count > 0 && dtCopy.Rows[0]["BookNo"] != DBNull.Value)
            {
                return dtCopy.Rows[0]["BookNo"].ToString().ToUpper();
            }
        }
    ...
    protected string FormatStatusBadge(object isRefObj, object isAdultsObj, object isChildrenObj, object availCopiesObj)
    {
        bool isRef = isRefObj != DBNull.Value && Convert.ToBoolean(isRefObj);
        bool isAdults = isAdultsObj != DBNull.Value && Convert.ToBoolean(isAdultsObj);
        bool isChildren = isChildrenObj != DBNull.Value && Convert.ToBoolean(isChildrenObj);
        int avail = availCopiesObj != DBNull.Value ? Convert.ToInt32(availCopiesObj) : 0;

        System.Text.StringBuilder sb = new System.Text.StringBuilder();

        if (isRef)
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #fee2e2; color: #991b1b; margin-right: 4px;'>REF</span> ");
        }
        else
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #e0f2fe; color: #0369a1; margin-right: 4px;'>CIRC</span> ");
        }

        if (isAdults && isChildren)
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #f3e8ff; color: #6b21a8; margin-right: 4px;'>GEN</span> ");
        }
        else if (isAdults)
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #ffedd5; color: #c2410c; margin-right: 4px;'>ADULTS</span> ");
        }
        else if (isChildren)
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #dcfce7; color: #15803d; margin-right: 4px;'>KIDS</span> ");
        }

        if (avail > 0)
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #d1fae5; color: #065f46;'>AVAIL (" + avail + ")</span>");
        }
        else
        {
            sb.Append("<span style='font-size: 10.5px; font-weight: 700; padding: 2px 6px; border-radius: 4px; background-color: #fee2e2; color: #991b1b;'>CHECKED OUT</span>");
        }
    ...
    {
        string coverFile = coverFileObj != null ? coverFileObj.ToString() : null;
        if (!string.IsNullOrEmpty(coverFile))
        {
            string coverUrl = ResolveUrl("~/Library Management/Images/BookCovers/" + coverFile);
            return "<img src=\"" + coverUrl + "\" alt=\"Book Cover\" style=\"width: 100%; height: 100%; object-fit: cover;\" onerror=\"this.style.display='none';this.nextElementSibling.style.display='block';\" /><div style=\"display:none; text-align: center; color: #94a3b8; font-size: 11px;\">No Cover</div>";
        }
        
        return @"<div style='text-align: center; color: #94a3b8; font-size: 11px;'>
                    <p style='font-size:10px;margin:0'>No Cover</p>
                 </div>";
    }

    protected string GetAvailabilityBadge(int available, int total)
    {
        if (total == 0)
        {
            return "<span style='font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 30px; background-color: #fee2e2; color: #991b1b;'>Out of stock (0 copies)</span>";
        }
        else if (available > 0)
        {
            return "<span style='font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 30px; background-color: #d1fae5; color: #065f46;'>Available: " + available + " / " + total + " copies</span>";
        }
        else
        {
            return "<span style='font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 30px; background-color: #fee2e2; color: #991b1b;'>All on Loan (" + total + " copies)</span>";
        }
    }
    ...
    public static object GetPublisherSuggestions(string query)
    {
        ...
        string sql = "SELECT TOP 10 PubName FROM Publishers WITH (NOLOCK) WHERE IsActive = 1 AND PubName LIKE N'%" + safeTerm + "%' ORDER BY PubName";
    ...
    public static object GetBookCopiesForPrinting(int bookID)
    {
        try
        {
            ...
            // Get Authors string
            System.Collections.Generic.List<string> authorsList = new System.Collections.Generic.List<string>();
            ...
            // Get Copies
            System.Collections.Generic.List<object> copies = new System.Collections.Generic.List<object>();
    ...
    [System.Web.Services.WebMethod]
    public static object GetBookFullDetail(int bookID)
    {
        DataSet ds = DBHelper.GetBookDetail(bookID);
        if (ds == null || ds.Tables.Count == 0 || ds.Tables[0].Rows.Count == 0)
        {
        ...
        DataRow b = ds.Tables[0].Rows[0];

        List<string> authorsList = new List<string>();
        if (ds.Tables.Count > 1)
        {
            foreach (DataRow ar in ds.Tables[1].Rows)
            {
                string role = ar.Table.Columns.Contains("RoleName") && ar["RoleName"] != DBNull.Value ? ar["RoleName"].ToString() : "";
                authorsList.Add(ar["FullName"].ToString() + (!string.IsNullOrEmpty(role) && role != "Author" ? " (" + role + ")" : ""));
            }
        }
        string authors = authorsList.Count > 0 ? string.Join(", ", authorsList) : "Unknown";

        List<object> copies = new List<object>();
        string primaryBookNo = "";
        if (ds.Tables.Count > 2)
        {
            foreach (DataRow cr in ds.Tables[2].Rows)
            {
                string copyBookNo = cr.Table.Columns.Contains("BookNo") && cr["BookNo"] != DBNull.Value ? cr["BookNo"].ToString() : "";
                if (string.IsNullOrEmpty(primaryBookNo) && !string.IsNullOrEmpty(copyBookNo))
                {
                    primaryBookNo = copyBookNo;
                }

                copies.Add(new {
                    CopyID = cr.Table.Columns.Contains("CopyID") && cr["CopyID"] != DBNull.Value ? cr["CopyID"].ToString() : "",
                    Barcode = cr.Table.Columns.Contains("Barcode") && cr["Barcode"] != DBNull.Value ? cr["Barcode"].ToString() : "",
                    BookNo = copyBookNo,
                    Condition = cr.Table.Columns.Contains("Condition") && cr["Condition"] != DBNull.Value ? cr["Condition"].ToString() : "New",
                    IsAvailable = cr.Table.Columns.Contains("IsAvailable") && cr["IsAvailable"] != DBNull.Value ? Convert.ToBoolean(cr["IsAvailable"]) : false,
                    ShelfAddress = cr.Table.Columns.Contains("ShelfAddress") && cr["ShelfAddress"] != DBNull.Value ? cr["ShelfAddress"].ToString() : (cr.Table.Columns.Contains("FullAddress") && cr["FullAddress"] != DBNull.Value ? cr["FullAddress"].ToString() : "Unassigned")
                });
            }
        }

        if (string.IsNullOrEmpty(primaryBookNo) && b.Table.Columns.Contains("AcqNo") && b["AcqNo"] != DBNull.Value)
        {
            primaryBookNo = b["AcqNo"].ToString();
        }

        bool isRef = b.Table.Columns.Contains("IsReference") && b["IsReference"] != DBNull.Value && Convert.ToBoolean(b["IsReference"]);
        bool isAdults = b.Table.Columns.Contains("IsAdults") && b["IsAdults"] != DBNull.Value && Convert.ToBoolean(b["IsAdults"]);
        bool isChildren = b.Table.Columns.Contains("IsChildren") && b["IsChildren"] != DBNull.Value && Convert.ToBoolean(b["IsChildren"]);

        string audience = (isAdults && isChildren) ? "ADULTS & CHILDREN" : isAdults ? "ADULTS ONLY" : isChildren ? "CHILDREN ONLY" : "GENERAL";
        string bookType = isRef ? "REFERENCE" : "CIRCULATING";

        int availCount = 0;
        foreach (dynamic cp in copies)
        {
            if (cp.IsAvailable) availCount++;
        }

        string rawCover = b.Table.Columns.Contains("CoverFile") && b["CoverFile"] != DBNull.Value ? b["CoverFile"].ToString().Trim() : "";
        string coverUrl = "";
        if (!string.IsNullOrEmpty(rawCover))
        {
            ...
            {
                coverUrl = System.Web.VirtualPathUtility.ToAbsolute(rawCover);
            }
            else if (rawCover.StartsWith("Images/"))
            {
                coverUrl = System.Web.VirtualPathUtility.ToAbsolute("~/Library Management/" + rawCover);
            }
            else
            {
                coverUrl = System.Web.VirtualPathUtility.ToAbsolute("~/Library Management/Images/BookCovers/" + rawCover);
            }
        }

        return new {
            BookID = b["BookID"],
            Title = b.Table.Columns.Contains("Title") && b["Title"] != DBNull.Value ? b["Title"].ToString() : "",
            SubTitle = b.Table.Columns.Contains("SubTitle") && b["SubTitle"] != DBNull.Value ? b["SubTitle"].ToString() : "",
            Authors = authors,
            CatName = b.Table.Columns.Contains("CatName") && b["CatName"] != DBNull.Value ? b["CatName"].ToString() : "",
            PubName = b.Table.Columns.Contains("PubName") && b["PubName"] != DBNull.Value ? b["PubName"].ToString() : "",
            LangName = b.Table.Columns.Contains("LangName") && b["LangName"] != DBNull.Value ? b["LangName"].ToString() : "",
            DDC = b.Table.Columns.Contains("DDC") && b["DDC"] != DBNull.Value ? b["DDC"].ToString() : "",
            ISBN13Fmt = b.Table.Columns.Contains("ISBN13Fmt") && b["ISBN13Fmt"] != DBNull.Value ? b["ISBN13Fmt"].ToString() : (b.Table.Columns.Contains("ISBN13") && b["ISBN13"] != DBNull.Value ? b["ISBN13"].ToString() : ""),
            ISBN10 = b.Table.Columns.Contains("ISBN10") && b["ISBN10"] != DBNull.Value ? b["ISBN10"].ToString() : "",
            PublishYear = b.Table.Columns.Contains("PublishYear") && b["PublishYear"] != DBNull.Value ? b["PublishYear"].ToString() : "",
            PublishingPlace = b.Table.Columns.Contains("PublishingPlace") && b["PublishingPlace"] != DBNull.Value ? b["PublishingPlace"].ToString() : "",
            Edition = b.Table.Columns.Contains("Edition") && b["Edition"] != DBNull.Value ? b["Edition"].ToString() : "",
            PageCount = b.Table.Columns.Contains("PageCount") && b["PageCount"] != DBNull.Value ? b["PageCount"].ToString() : "",
            CoverFile = rawCover,
            CoverUrl = coverUrl,
            Synopsis = b.Table.Columns.Contains("Synopsis") && b["Synopsis"] != DBNull.Value ? b["Synopsis"].ToString() : "",
            BookNo = primaryBookNo,
            BookType = bookType,
            Audience = audience,
            TotalCopies = copies.Count.ToString(),
            AvailableCopies = availCount.ToString(),
            Copies = copies
        };
    }
...
public static class DBHelper
{
    ...
    public static DataTable SearchBooks(string term, short? catID, byte? langID, short? pubID, short? yearFrom, short? yearTo, bool availOnly, short? rackID, int? pageNumber = null, int? pageSize = null, string ddc = null, int? bookNo = null)
    {
        var prms = new[]
        {
            new SqlParameter("@Term",      SqlDbType.NVarChar, 200) { Value = (object)term ?? DBNull.Value },
            ...
            new SqlParameter("@DDC",        SqlDbType.VarChar, 50) { Value = (object)ddc ?? DBNull.Value },
            new SqlParameter("@BookNo",     SqlDbType.Int) { Value = (object)bookNo ?? DBNull.Value }
        };
        ...
        else
        {
            ...
            string query = @"
                ...
                FROM (
                    ...
                    WHERE IsActive = '1' 
                      AND (MemberNo LIKE N'%" + cleanSearch + @"%' OR MemberName LIKE N'%" + cleanSearch + @"%')
                    ...
                    WHERE mp.IsActive = '1' 
                      ...
                      AND (ms.MembershipNo LIKE N'%" + cleanSearch + @"%' OR ms.SpouseName LIKE N'%" + cleanSearch + @"%')
                    ...
                    WHERE mp.IsActive = '1' 
                      ...
                      AND (mc.MembershipNo LIKE N'%" + cleanSearch + @"%' OR mc.ChildName LIKE N'%" + cleanSearch + @"%')
                ) AS Combined
        ...
        byte? langID, short? year, string ddc = null)
    {
        var prms = new[]
        {
            ...
            new SqlParameter("@BookName",  SqlDbType.NVarChar, 200) { Value = (object)bookName ?? DBNull.Value },
            new SqlParameter("@Edition",   SqlDbType.NVarChar, 50) { Value = (object)edition ?? DBNull.Value },
            new SqlParameter("@PubID",     (object)pubID    ?? DBNull.Value),
            new SqlParameter("@CatID",     (object)catID    ?? DBNull.Value),
            new SqlParameter("@LangID",    (object)langID   ?? DBNull.Value),
            new SqlParameter("@Year",      (object)year     ?? DBNull.Value),
            new SqlParameter("@DDC",       SqlDbType.NVarChar, 100) { Value = (object)ddc ?? DBNull.Value }
        };

...
public partial class Pages_Books_BookSearch : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            BindFilterDropdowns();
            litResultsHeader.Text = "Please enter search criteria above to search the library catalogue.";
        }
    }

    private void BindFilterDropdowns()
    {
        // 1. Categories (9 DDC Classes)
        ddlCategory.DataSource = DBHelper.GetCategories();
        ddlCategory.DataTextField = "CatName";
        ddlCategory.DataValueField = "CatID";
        ddlCategory.DataBind();
        ddlCategory.Items.Insert(0, new ListItem("- All Subject Categories -", "0"));
    ...
    {
        string term = !string.IsNullOrEmpty(txtGlobalSearch.Text) ? txtGlobalSearch.Text.Trim() : null;
        string author = !string.IsNullOrEmpty(txtAuthor.Text) ? txtAuthor.Text.Trim() : null;
        string bookName = !string.IsNullOrEmpty(txtBookName.Text) ? txtBookName.Text.Trim() : null;
        ...
        string publisherName = !string.IsNullOrEmpty(txtPublisher.Text) ? txtPublisher.Text.Trim() : null;
        ...
        {
            short tempYear;
            if (short.TryParse(txtYear.Text, out tempYear))
            {
                year = tempYear;
            }
        }

        // Do not perform search if all filters are empty
        if (term == null && author == null && bookName == null && edition == null &&
            publisherName == null && catID == null && langID == null && year == null && ddc == null)
        {
            rptBooks.DataSource = null;
            rptBooks.DataBind();
            rptBooks.Visible = false;
            pnlEmptyState.Visible = false;
            litResultsHeader.Text = "Please enter at least one search criterion to perform a search.";
            return;
        }

        DataTable dt = Pages_Books_BookSearch.DBHelper.SearchBooksAdvanced(term, author, bookName, edition, null, catID, langID, year, ddc, null);

        // Filter by manual publisher name if provided
        if (dt != null && !string.IsNullOrEmpty(publisherName))
        {
            DataTable filteredDt = dt.Clone();
            foreach (DataRow row in dt.Rows)
            {
                string pubName = row["PubName"] != DBNull.Value ? row["PubName"].ToString() : "";
                if (pubName.IndexOf(publisherName, StringComparison.OrdinalIgnoreCase) >= 0)
                {
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            pnlEmptyState.Visible = false;
            litResultsHeader.Text = "Search Results (" + dt.Rows.Count + " books found)";
        }
        else
        {
            rptBooks.Visible = false;
            pnlEmptyState.Visible = true;
            litResultsHeader.Text = "No Results Found";
        }
    }
    ...
    protected void lnkClearAll_Click(object sender, EventArgs e)
    {
        ...
        rptBooks.Visible = false;
        pnlEmptyState.Visible = false;
        pnlBookDetailModal.Visible = false;
        litResultsHeader.Text = "Please enter search criteria above to search the library catalogue.";
    }
    ...
    protected void rptBooks_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            int bookID = Convert.ToInt32(e.CommandArgument);
            LoadBookModalDetails(bookID);
        }
    }

    protected string GetCategoryDisplay(object isRefObj, object isAdultsObj, object isChildrenObj)
    {
        bool isRef = isRefObj != null && isRefObj != DBNull.Value && Convert.ToBoolean(isRefObj);
        bool isAdults = isAdultsObj != null && isAdultsObj != DBNull.Value && Convert.ToBoolean(isAdultsObj);
        bool isChildren = isChildrenObj != null && isChildrenObj != DBNull.Value && Convert.ToBoolean(isChildrenObj);

        List<string> parts = new List<string>();
        if (isRef) parts.Add("REFERENCE");

        if (isAdults && isChildren)
        {
            parts.Add("ADULTS & CHILDREN");
        }
        else if (isAdults)
        {
            parts.Add("ADULTS");
        ...
        if (parts.Count == 0) return "GENERAL";
        return string.Join(" / ", parts);
    }

    private void LoadBookModalDetails(int bookID)
    {
        DataSet ds = DBHelper.GetBookDetail(bookID);
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
        {
            DataRow r = ds.Tables[0].Rows[0];
            litModalTitle.Text = r["Title"] != DBNull.Value ? r["Title"].ToString().ToUpper() : "BOOK DETAILS";
            
            string coverFile = r["CoverFile"] != DBNull.Value ? r["CoverFile"].ToString() : "";
            if (!string.IsNullOrEmpty(coverFile))
            {
                imgModalCover.ImageUrl = ResolveUrl("~/Library Management/Images/BookCovers/" + coverFile);
            }
            else
            {
                imgModalCover.ImageUrl = ResolveUrl("~/Library Management/Images/BookCovers/no-cover.png");
            }

            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            
            // RS2: Authors (ds.Tables[1])
            if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
            {
                List<string> authorsList = new List<string>();
                foreach (DataRow aRow in ds.Tables[1].Rows)
                {
                    if (aRow["FullName"] != DBNull.Value)
                    {
                        authorsList.Add(aRow["FullName"].ToString().ToUpper());
                    }
                }
                if (authorsList.Count > 0)
                {
                    sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>AUTHOR(S):</strong> " + string.Join(", ", authorsList) + "</div>");
                }
            }

            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>SUBTITLE:</strong> " + (r["SubTitle"] != DBNull.Value ? r["SubTitle"].ToString().ToUpper() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>DDC CALL NO:</strong> " + (r["DDC"] != DBNull.Value ? r["DDC"].ToString().ToUpper() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>SUBJECT:</strong> " + (r["CatName"] != DBNull.Value ? r["CatName"].ToString().ToUpper() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>CATEGORY:</strong> " + GetCategoryDisplay(r["IsReference"], r["IsAdults"], r["IsChildren"]) + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>PUBLISHER:</strong> " + (r["PubName"] != DBNull.Value ? r["PubName"].ToString().ToUpper() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>PUBLISH YEAR:</strong> " + (r["PublishYear"] != DBNull.Value ? r["PublishYear"].ToString() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>EDITION:</strong> " + (r["Edition"] != DBNull.Value ? r["Edition"].ToString().ToUpper() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>ISBN-13:</strong> " + (r["ISBN13Fmt"] != DBNull.Value ? r["ISBN13Fmt"].ToString() : "-") + "</div>");
            sb.Append("<div style='margin-bottom: 8px;'><strong style='color:#0f1e36;'>LANGUAGE:</strong> " + (r["LangName"] != DBNull.Value ? r["LangName"].ToString().ToUpper() : "-") + "</div>");

            // RS3: Physical Copies (ds.Tables[2])
            if (ds.Tables.Count > 2 && ds.Tables[2].Rows.Count > 0)
            {
                sb.Append("<div style='margin-top: 16px; border-top: 1px solid #cbd5e1; padding-top: 12px;'><strong style='color:#0f1e36;'>PHYSICAL COPIES (" + ds.Tables[2].Rows.Count + "):</strong>");
                sb.Append("<table style='width: 100%; border-collapse: collapse; margin-top: 8px; font-size: 12px;'><thead><tr style='background: #f1f5f9; text-align: left;'><th style='padding: 6px 8px;'>BOOK NO</th><th style='padding: 6px 8px;'>BARCODE</th><th style='padding: 6px 8px;'>CONDITION</th><th style='padding: 6px 8px;'>STATUS</th></tr></thead><tbody>");
                foreach (DataRow cpRow in ds.Tables[2].Rows)
                {
                    bool isAvail = cpRow.Table.Columns.Contains("IsAvailable") && cpRow["IsAvailable"] != DBNull.Value && Convert.ToBoolean(cpRow["IsAvailable"]);
                    sb.Append("<tr style='border-bottom: 1px solid #e2e8f0;'>");
                    sb.Append("<td style='padding: 6px 8px; font-weight: bold;'>" + (cpRow.Table.Columns.Contains("BookNo") && cpRow["BookNo"] != DBNull.Value ? cpRow["BookNo"].ToString() : "-") + "</td>");
                    sb.Append("<td style='padding: 6px 8px;'>" + (cpRow.Table.Columns.Contains("Barcode") && cpRow["Barcode"] != DBNull.Value ? cpRow["Barcode"].ToString() : "-") + "</td>");
                    sb.Append("<td style='padding: 6px 8px;'>" + (cpRow.Table.Columns.Contains("CondName") && cpRow["CondName"] != DBNull.Value ? cpRow["CondName"].ToString().ToUpper() : "-") + "</td>");
                    sb.Append("<td style='padding: 6px 8px; font-weight: bold; color:" + (isAvail ? "#16a34a" : "#ef4444") + ";'>" + (isAvail ? "AVAILABLE" : "ON LOAN") + "</td>");
                    sb.Append("</tr>");
                }
            ...
            pnlBookDetailModal.Visible = true;
        }
    }
    ...
    protected void rptBooks_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        {
            Repeater rptCopies = (Repeater)e.Item.FindControl("rptCopies");
            if (rptCopies != null)
            {
    ...
    #region AutoComplete WebMethods

    [WebMethod]
    public static object GetGlobalSearchSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 2) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = @"SELECT DISTINCT TOP 10 b.Title 
                       FROM Books b WITH (NOLOCK) 
                       WHERE b.IsActive = 1 
                         AND (b.Title LIKE N'%" + safeTerm + @"%' 
                              OR b.ISBN13 LIKE N'%" + safeTerm + @"%' 
                              OR EXISTS (SELECT 1 FROM BookAuthors ba JOIN Authors a ON ba.AuthorID = a.AuthorID WHERE ba.BookID = b.BookID AND a.FullName LIKE N'%" + safeTerm + @"%')) 
                       ORDER BY b.Title";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
            if (row["Title"] != DBNull.Value) list.Add(row["Title"].ToString());
        }
        return list;
    }
    ...
    public static object GetAuthorSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 2) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = "SELECT DISTINCT TOP 10 FullName FROM Authors WITH (NOLOCK) WHERE IsActive = 1 AND FullName LIKE N'%" + safeTerm + "%' ORDER BY FullName";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
            if (row["FullName"] != DBNull.Value) list.Add(row["FullName"].ToString());
        }
        return list;
    }

    [WebMethod]
    public static object GetBookTitleSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 2) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = "SELECT DISTINCT TOP 10 Title FROM Books WITH (NOLOCK) WHERE IsActive = 1 AND Title LIKE N'%" + safeTerm + "%' ORDER BY Title";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
    ...
    [WebMethod]
    public static object GetEditionSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 1) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = "SELECT DISTINCT TOP 10 Edition FROM Books WITH (NOLOCK) WHERE IsActive = 1 AND Edition IS NOT NULL AND Edition <> '' AND Edition LIKE N'%" + safeTerm + "%' ORDER BY Edition";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
            if (row["Edition"] != DBNull.Value) list.Add(row["Edition"].ToString());
        }
        return list;
    }
    ...
    public static object GetPublisherSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 2) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = "SELECT DISTINCT TOP 10 PubName FROM Publishers WITH (NOLOCK) WHERE IsActive = 1 AND PubName LIKE N'%" + safeTerm + "%' ORDER BY PubName";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
            if (row["PubName"] != DBNull.Value) list.Add(row["PubName"].ToString());
        }
        return list;
    }

    [WebMethod]
    public static object GetDdcSuggestions(string query)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Trim().Length < 1) return new List<string>();
        string safeTerm = query.Trim().Replace("'", "''");
        string sql = "SELECT DISTINCT TOP 10 DDC FROM Books WITH (NOLOCK) WHERE IsActive = 1 AND DDC IS NOT NULL AND DDC <> '' AND DDC LIKE N'%" + safeTerm + "%' ORDER BY DDC";
        DataTable dt = DBHelper.GetTableData(sql);
        var list = new List<string>();
        foreach (DataRow row in dt.Rows)
        {
            if (row["DDC"] != DBNull.Value) list.Add(row["DDC"].ToString());
        }
        return list;
    }
...
/// All communication uses Stored Procedures ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no inline SQL.
...
public static class DBHelper
{
    ...
    // ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    //  Execute SP ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ DataTable  (SELECT results)
    // ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    ...
    // Execute SP ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ DataSet  (multiple result sets, e.g. sp_GetBookDetail)
    ...
    // ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    //  Execute SP ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ no return value (fire-and-forget DML)
    // ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    ...
    {
        return GetCachedTable("Categories_MainDDC", "SELECT CatID, CatName FROM Categories WHERE IsActive = 1 AND ParentCatID IS NULL ORDER BY CatName");
    }
    ...
    public static DataTable SearchBooksAdvanced(
        ...
        byte? langID, short? year, string ddc = null, int? bookNo = null)
    {
        var prms = new[]
        {
            new SqlParameter("@Term",      SqlDbType.NVarChar, 200) { Value = (object)term ?? DBNull.Value },
...
{
    // Strip hyphens and spaces Ã¢â€ â€™ clean 13-digit string
    ...
    /// <summary>Generate a barcode string for a physical copy: ISBN13-001, ISBN13-002Ã¢â‚¬Â¦</summary>

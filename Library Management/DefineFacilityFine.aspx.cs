...
public partial class Pages_System_DefineFacilityFine : System.Web.UI.Page
{
    ...
    private void BindFineReasons()
    {
        DataTable dt = DBHelper.GetFineReasons();
        gvFineReasons.DataSource = dt;
        gvFineReasons.DataBind();
    }
    ...
    protected void btnSaveFacility_Click(object sender, EventArgs e)
    {
        ...
        {
            ShowAlert("Facility Name is required.", "alert-error");
            return;
        }

        if (!decimal.TryParse(txtCostPerHour.Text, out costPerHour) || costPerHour < 0)
        {
            ShowAlert("Cost Per Hour must be a valid non-negative number.", "alert-error");
            return;
        }

        int? facilityID = null;
        if (!string.IsNullOrEmpty(hfFacilityID.Value))
        {
            facilityID = Convert.ToInt32(hfFacilityID.Value);
        }

        bool isActive = chkFacilityActive.Checked;

        string result = DBHelper.DefineFacility(facilityID, facilityName, costPerHour, isActive);
    ...
    protected void btnClearFacility_Click(object sender, EventArgs e)
    {
        ResetFacilityForm();
    }
    ...
    protected void gvFacilities_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            int facilityID = Convert.ToInt32(e.CommandArgument);
            DataTable dt = DBHelper.GetFacilities();
            
            // Find row in DataTable
            DataRow[] rows = dt.Select("FacilityID = " + facilityID);
            if (rows.Length > 0)
            {
                DataRow r = rows[0];
                hfFacilityID.Value = r["FacilityID"].ToString();
                txtFacilityName.Text = r["FacilityName"].ToString();
                txtCostPerHour.Text = Convert.ToDecimal(r["CostPerHour"]).ToString("0.00");
                chkFacilityActive.Checked = Convert.ToBoolean(r["IsActive"]);

                litFacilityFormTitle.Text = "Edit Facility Details";
                btnClearFacility.Visible = true;
            }
        }
    }
    ...
    private void ResetFacilityForm()
    {
        ...
        txtCostPerHour.Text = "";
        chkFacilityActive.Checked = true;
        litFacilityFormTitle.Text = "Add New Facility";
        btnClearFacility.Visible = false;
    }

    // --------------------------------------------------------------
    //  Fine Reasons Management Handlers
    // --------------------------------------------------------------

    protected void btnSaveFineReason_Click(object sender, EventArgs e)
    {
        string reasonName = txtReasonName.Text.Trim();
        decimal defaultAmount = 0;

        if (string.IsNullOrEmpty(reasonName))
        {
            ShowAlert("Fine Reason Name is required.", "alert-error");
            return;
        }

        if (!decimal.TryParse(txtDefaultAmount.Text, out defaultAmount) || defaultAmount < 0)
        {
            ShowAlert("Default Fine Amount must be a valid non-negative number.", "alert-error");
            return;
        }

        byte? reasonID = null;
        if (!string.IsNullOrEmpty(hfReasonID.Value))
        {
            reasonID = Convert.ToByte(hfReasonID.Value);
        }

        string result = DBHelper.DefineFineReason(reasonID, reasonName, defaultAmount);
    ...
    protected void btnClearFineReason_Click(object sender, EventArgs e)
    {
        ResetFineReasonForm();
    }

    protected void gvFineReasons_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "EditFineReason")
        {
            byte reasonID = Convert.ToByte(e.CommandArgument);
            DataTable dt = DBHelper.GetFineReasons();
            ...
            {
                DataRow r = rows[0];
                hfReasonID.Value = r["ReasonID"].ToString();
                txtReasonName.Text = r["ReasonName"].ToString();
                txtDefaultAmount.Text = Convert.ToDecimal(r["DefaultAmount"]).ToString("0.00");

                litFineFormTitle.Text = "Edit Fine Type Details";
                btnClearFineReason.Visible = true;
            }
        }
    }

    private void ResetFineReasonForm()
    {
        hfReasonID.Value = "";
        txtReasonName.Text = "";
        txtDefaultAmount.Text = "";
        litFineFormTitle.Text = "Add Fine Type";
        btnClearFineReason.Visible = false;
    }

...
{
    public partial class WeededOutReport : System.Web.UI.Page
    {
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DateTime.TryParse(txtToDate.Text, out parsedDate))
                {
                    toDate = parsedDate;
                }
                ...
                DataTable dt = DBHelper.GetWeedLogReport(fromDate, toDate, search, actionType);
                if (dt != null && dt.Rows.Count > 0)
                {
                    RenderPrintView(dt, "Weeding & Restoration Audit Report");
                }
                else
                {
                    Response.Write("<script>alert('No records found to print.');</script>");
                }
            }
            catch (System.Threading.ThreadAbortException)
            {
                // Do nothing, normal behavior when Response.End is called
            }
            catch (Exception ex)
            {
                Response.Write("<script>alert('Error generating print view: " + ex.Message.Replace("'", "\\'") + "');</script>");
            }
        }
        ...
        private void RenderPrintView(DataTable dt, string title)
        {
            ...
            using (System.IO.StringWriter sw = new System.IO.StringWriter())
            {
                using (System.Web.UI.HtmlTextWriter hw = new System.Web.UI.HtmlTextWriter(sw))
                {
                    ...
                    hw.Write(".header { border-bottom: 2px solid #0f1e36; padding-bottom: 10px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: flex-end; }");
                    hw.Write(".header-title { display: flex; flex-direction: column; }");
                    hw.Write(".header h1 { color: #0f1e36; margin: 0; font-size: 22px; font-weight: 700; }");
                    hw.Write(".meta { font-size: 11px; color: #64748b; margin-top: 5px; }");
                    hw.Write("table { width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 12px; }");
                    hw.Write("th { background-color: #f1f5f9; color: #475569; font-weight: 700; text-align: left; padding: 8px 10px; border: 1px solid #cbd5e1; }");
                    hw.Write("td { padding: 8px 10px; border: 1px solid #cbd5e1; vertical-align: top; }");
                    hw.Write("tr:nth-child(even) { background-color: #f8fafc; }");
                    hw.Write(".status-weed { display: inline-block; padding: 3px 6px; border-radius: 4px; font-size: 10px; font-weight: 700; background-color: #fee2e2; color: #991b1b; text-transform: uppercase; }");
                    hw.Write(".status-restore { display: inline-block; padding: 3px 6px; border-radius: 4px; font-size: 10px; font-weight: 700; background-color: #d1fae5; color: #065f46; text-transform: uppercase; }");
                    hw.Write("</style></head><body onload='window.print();'>");
                    hw.Write("<div class='header'><div class='header-title'><h1>" + title + "</h1>");
                    hw.Write("<div class='meta'>Generated on: " + DateTime.Now.ToString("dd-MMM-yyyy hh:mm tt") + " | Operator: " + (Session["StaffName"] ?? "LGC Staff") + "</div></div>");
                    hw.Write("<img src='" + ResolveUrl("~/Library Management/Images/logo_new.png") + "' alt='Logo' style='height: 50px;' /></div>");

                    hw.Write("<table><thead><tr>");
                    hw.Write("<th style='width: 40px;'>SR#</th><th>Book Details</th><th style='width: 100px;'>Action</th><th>Transition Details</th><th>Remarks / Reasons</th><th>Actioned By</th>");
                    hw.Write("</tr></thead><tbody>");

                    int sr = 1;
                    foreach (DataRow r in dt.Rows)
                    {
                        string bookTitle = r["BookTitle"] != DBNull.Value ? r["BookTitle"].ToString() : "";
                        string bookNo = r["BookNo"] != DBNull.Value ? r["BookNo"].ToString() : "";
                        string actionType = r["ActionType"] != DBNull.Value ? r["ActionType"].ToString() : "";
                        string remarks = r["Remarks"] != DBNull.Value ? r["Remarks"].ToString() : "";
                        
                        string oldCond = r["OldCondition"] != DBNull.Value ? r["OldCondition"].ToString() : "";
                        string newCond = r["NewCondition"] != DBNull.Value ? r["NewCondition"].ToString() : "";
                        string oldLoc = r["OldLocation"] != DBNull.Value ? r["OldLocation"].ToString() : "";
                        string newLoc = r["NewLocation"] != DBNull.Value ? r["NewLocation"].ToString() : "";
                        
                        string actionedBy = r["ActionedByStaff"] != DBNull.Value ? r["ActionedByStaff"].ToString() : "";
                        string actionedAt = r["ActionedAt"] != DBNull.Value ? Convert.ToDateTime(r["ActionedAt"]).ToString("dd-MMM-yyyy hh:mm tt") : "";

                        string actionBadge = actionType == "WEED" 
                            ? "<span class='status-weed'>Weeded Out</span>" 
                            : "<span class='status-restore'>Restored</span>";

                        hw.Write("<tr>");
                        hw.Write("<td>" + sr++ + "</td>");
                        hw.Write("<td><strong>" + bookTitle + "</strong><br/><span style='font-size: 11px; color:#64748b;'>Book No: " + bookNo + "</span></td>");
                        hw.Write("<td>" + actionBadge + "</td>");
                        hw.Write("<td><div style='font-size: 11px;'><strong>Condition:</strong> " + oldCond + " -> " + newCond + "<br/><strong>Location:</strong> " + oldLoc + " -> " + newLoc + "</div></td>");
                        hw.Write("<td>" + remarks + "</td>");
                        hw.Write("<td><strong>" + actionedBy + "</strong><br/><span style='font-size: 10px; color:#94a3b8;'>" + actionedAt + "</span></td>");
                        hw.Write("</tr>");
                    }
        ...
        private void BindLogGrid()
        {
            try
            {
                ...
                gvLog.DataSource = formatted;
                gvLog.DataBind();
            }
            catch (Exception ex)
            {
                // Log/handle error
                Response.Write("<script>alert('Error loading weeding report: " + ex.Message.Replace("'", "\\'") + "');</script>");
            }
        }

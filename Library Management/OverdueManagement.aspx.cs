...
public partial class Pages_OverdueManagement : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            string tab = Request.QueryString["tab"] ?? "REMINDERS";
            string scenario = Request.QueryString["scenario"] ?? "0";
            
            if (ddlScenarioFilter.Items.FindByValue(scenario) != null)
            {
                ddlScenarioFilter.SelectedValue = scenario;
            }
            
            SetActiveTab(tab.ToUpper());
            BindRemindersGrid();
            BindReversalsGrid();
        }
    }
    ...
    private void SetActiveTab(string tab)
    {
        ...
        pnlTabReminders.Visible = (tab == "REMINDERS");
        pnlTabReversals.Visible = (tab == "REVERSALS");

        string activeStyle = "padding: 18px 24px; text-align: center; background: none; border: none; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #c5a059; border-bottom: 3px solid #c5a059; cursor: pointer; transition: all 0.25s ease; outline: none;";
        string inactiveStyle = "padding: 18px 24px; text-align: center; background: none; border: none; font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #64748b; border-bottom: 3px solid transparent; cursor: pointer; transition: all 0.25s ease; outline: none;";

        btnTabReminders.Attributes["style"] = (tab == "REMINDERS" ? activeStyle : inactiveStyle);
        btnTabReversals.Attributes["style"] = (tab == "REVERSALS" ? activeStyle : inactiveStyle);
    }

    // =========================================================================
    //  GRID BINDINGS & SEARCHES
    // =========================================================================
    private void BindRemindersGrid()
    {
        int scenario = Convert.ToInt32(ddlScenarioFilter.SelectedValue);
        DataTable dt = DBHelper.GetOverdueReminderList(scenario == 0 ? (int?)null : scenario);
        
        string searchText = txtSearchReminders.Text.Trim();
        if (!string.IsNullOrEmpty(searchText))
        {
            DataTable dtFiltered = dt.Clone();
            var matchedMemberIDs = new System.Collections.Generic.HashSet<int>();
            try
            {
                string cleanSearch = searchText;
                if (cleanSearch.Contains(" - "))
                {
                    cleanSearch = cleanSearch.Split(new string[] { " - " }, StringSplitOptions.None)[0].Trim();
                }
                DataTable dtMatches = DBHelper.GetMembers(cleanSearch);
                foreach (DataRow r in dtMatches.Rows)
                {
                    matchedMemberIDs.Add(Convert.ToInt32(r["MemberID"]));
                }
            }
            catch {}
            ...
            {
                int rowMemberID = Convert.ToInt32(row["MemberID"]);
                if (row["MemberName"].ToString().IndexOf(searchText, StringComparison.OrdinalIgnoreCase) >= 0 ||
                    row["MembershipNo"].ToString().IndexOf(searchText, StringComparison.OrdinalIgnoreCase) >= 0 ||
                    row["BookTitle"].ToString().IndexOf(searchText, StringComparison.OrdinalIgnoreCase) >= 0 ||
                    row["Barcode"].ToString().IndexOf(searchText, StringComparison.OrdinalIgnoreCase) >= 0 ||
                    matchedMemberIDs.Contains(rowMemberID))
                {
                    dtFiltered.ImportRow(row);
                }
            }
            gvReminders.DataSource = dtFiltered;
        }
        else
        {
    ...
    private void BindReversalsGrid()
    {
        DataTable dt = DBHelper.GetFinalChargedLoans();
        string searchText = txtSearchReversals.Text.Trim();
        ...
        if (!string.IsNullOrEmpty(searchText))
        {
            ...
            gvReversals.DataSource = dtFiltered;
        }
        else
        {
            gvReversals.DataSource = dt;
        }

        gvReversals.DataBind();
    }

    protected void ddlScenarioFilter_SelectedIndexChanged(object sender, EventArgs e)
    {
        BindRemindersGrid();
    }

    protected void txtSearchReminders_TextChanged(object sender, EventArgs e)
    {
        BindRemindersGrid();
    }
    ...
    {
        BindRemindersGrid();
        ShowAlert("Reminders queue refreshed.", true);
    }

    protected void txtSearchReversals_TextChanged(object sender, EventArgs e)
    {
        BindReversalsGrid();
    }

    protected void btnRefreshReversals_Click(object sender, EventArgs e)
    {
        BindReversalsGrid();
        ShowAlert("Reversals queue refreshed.", true);
    }

    protected void gvReminders_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        gvReminders.PageIndex = e.NewPageIndex;
        BindRemindersGrid();
    }

    protected void gvReversals_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        gvReversals.PageIndex = e.NewPageIndex;
        BindReversalsGrid();
    }

    // =========================================================================
    //  REMINDERS QUEUE ROW COMMANDS (MARK SENT & LETTER PREVIEW)
    // =========================================================================
    protected void gvReminders_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "GenerateLetter")
        {
            int index = Convert.ToInt32(e.CommandArgument);
            if (index >= 0 && index < gvReminders.Rows.Count)
            {
                int loanID = Convert.ToInt32(gvReminders.DataKeys[index].Value);
                LoadLetterPreview(loanID);
            }
        }
        else if (e.CommandName == "MarkSent")
        {
            int loanID = Convert.ToInt32(e.CommandArgument);
            ProcessMarkSent(loanID);
        }
    }

    private void LoadLetterPreview(int loanID)
    {
        // Query reminder list for this specific Loan ID
        DataTable dt = DBHelper.GetOverdueReminderList(null);
        DataRow[] rows = dt.Select("LoanID = " + loanID);
        if (rows.Length > 0)
        {
            DataRow row = rows[0];
            int scenario = Convert.ToInt32(row["ApplicableScenario"]);

            litLetterRef.Text = loanID.ToString() + "/" + DateTime.Today.ToString("yyyyMM");
            litLetterMemberName.Text = row["MemberName"].ToString();
            litLetterMemberNo.Text = row["MembershipNo"].ToString();
            litLetterMemberAddress.Text = row["MemberAddress"].ToString();
            litLetterBookTitle.Text = row["BookTitle"].ToString();
            litLetterBarcode.Text = row["Barcode"].ToString();
            litLetterIssueDate.Text = Convert.ToDateTime(row["IssueDate"]).ToString("dd-MMM-yyyy");
            litLetterDueDate.Text = Convert.ToDateTime(row["DueDate"]).ToString("dd-MMM-yyyy");
            litLetterOverdueDays.Text = row["DaysOverdue"].ToString();

            // Custom Subject & Templates
            if (scenario == 1)
            {
                litLetterSubject.Text = "First Overdue Notice: Gentle Reminder";
                litLetterMainBody.Text = "This is a gentle reminder that the library book mentioned below is currently overdue. We understand that it is easy to lose track of dates, and we kindly ask that you return it to the Lahore Gymkhana Club Library at your earliest convenience so other members may borrow it.";
                phFinancialDetails.Visible = false;
            }
            else if (scenario == 2)
            {
                litLetterSubject.Text = "Second Overdue Notice: Urgent Return Request";
                litLetterMainBody.Text = "We are writing to draw your attention to our previous reminder regarding the overdue library book listed below. This item has now exceeded its due date by 15 days or more. Please return the book immediately to avoid further overdue fines and to ensure your borrowing privileges remain active.";
                phFinancialDetails.Visible = false;
            }
            else if (scenario == 3)
            {
                litLetterSubject.Text = "Final Overdue Notice: Account Charge Warning";
                litLetterMainBody.Text = "<strong>CRITICAL NOTICE:</strong> Despite two prior notices, the library book listed below has not been returned and is now overdue by 30 days or more. As per Lahore Gymkhana Club Library rules, we have automatically billed your member account for the replacement cost of the book, a 200% penalty charge, and standard overdue days fine. These library charges have been posted directly to your ledger.";
                
                phFinancialDetails.Visible = true;
                litDaysFineCount.Text = row["DaysOverdue"].ToString();
                litDaysFineAmt.Text = Convert.ToDecimal(row["EstFine"]).ToString("N2");
                litBookCostAmt.Text = Convert.ToDecimal(row["AcqCost"]).ToString("N2");
                litPenaltyAmt.Text = Convert.ToDecimal(row["PenaltyAmount"]).ToString("N2");
                litTotalLiabAmt.Text = Convert.ToDecimal(row["TotalOverdueCharge"]).ToString("N2");
            }

            litPrintableLetter.Text = GenerateLetterHtml(row, loanID, scenario);
            pnlLetterModal.Visible = true;
        }
        else
        {
            ShowAlert("Error: Details not found for this overdue loan.", false);
        }
    }
    ...
    private static string GetOrdinalDate(DateTime date)
    {
        ...
        return day + suffix + " " + date.ToString("MMMM, yyyy");
    }

    private static string FormatAddress(string address)
    {
        if (string.IsNullOrEmpty(address)) return "LAHORE";
        string clean = address.ToUpper();
    ...
    private string GenerateLetterHtml(DataRow row, int loanID, int scenario)
    {
        string membershipNo = row["MembershipNo"].ToString();
        string memberName = row["MemberName"].ToString().ToUpper();
        string rawAddress = row["MemberAddress"].ToString();
        string memberAddress = FormatAddress(rawAddress);
        string bookTitle = row["BookTitle"].ToString().ToUpper();
        string barcode = row["Barcode"].ToString();
        string bookNo = "";
        if (row.Table.Columns.Contains("AcqNo") && row["AcqNo"] != DBNull.Value && !string.IsNullOrEmpty(row["AcqNo"].ToString()))
        {
            bookNo = row["AcqNo"].ToString().Trim();
        }
        else if (row.Table.Columns.Contains("BookID") && row["BookID"] != DBNull.Value)
        {
            bookNo = Convert.ToInt32(row["BookID"]).ToString("D6");
        }
        else
        {
            bookNo = barcode; // fallback
        }
        string ddc = row.Table.Columns.Contains("DDC") && row["DDC"] != DBNull.Value ? row["DDC"].ToString().Trim() : "N/A";
        string issueDate = Convert.ToDateTime(row["IssueDate"]).ToString("dd/MM/yyyy");
        string dueDate = Convert.ToDateTime(row["DueDate"]).ToString("dd/MM/yyyy");
        string currentDate = DateTime.Today.ToString("dd/MM/yyyy");
        string currentTime = DateTime.Now.ToString("h:mm:sstt").ToUpper(); // e.g. 5:53:23PM
        string staffName = GetStaffName(CurrentStaffID);
        string auditStamp = staffName + ", " + currentDate + ", " + currentTime + ", LM00." + string.Format("{0:D2}", loanID);

        StringBuilder sb = new StringBuilder();

        if (scenario == 1)
        {
            sb.Append("<div style='font-family: Arial, sans-serif; color: #000; line-height: 1.5; padding: 10px; text-align: left;'>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 20px;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='width: 80px; vertical-align: top; padding: 0;'>");
            sb.Append("        <img src='/Library Management/Images/logo_new.png' alt='Lahore Gymkhana' style='height: 75px;' />");
            sb.Append("      </td>");
            sb.Append("      <td style='vertical-align: top; padding-left: 15px; text-align: left;'>");
            sb.Append("        <div style='font-size: 22px; font-weight: bold; letter-spacing: 0.5px;'>LAHORE GYMKHANA</div>");
            sb.Append("        <div style='font-size: 12px; color: #000; margin-top: 2px;'>Upper Shahrah-e-Quaid-e-Azam, Lahore</div>");
            sb.Append("      </td>");
            sb.Append("    </tr>");
            sb.Append("  </table>");
            sb.Append("  <div style='text-align: center; margin: 30px 0 20px 0;'>");
            sb.Append("    <span style='font-size: 20px; font-weight: bold; text-decoration: underline; letter-spacing: 0.5px;'>FIRST REMINDER</span>");
            sb.Append("  </div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 13px;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='vertical-align: top; width: 50%;'>");
            sb.Append("        <table style='border-collapse: collapse;'>");
            sb.Append("          <tr><td style='padding: 2px 0; width: 100px;'><strong>Reminder No:</strong></td><td style='padding: 2px 0;'>1-" + string.Format("{0:D6}", loanID) + " / " + membershipNo + "</td></tr>");
            sb.Append("          <tr><td style='padding: 2px 0;'><strong>Dated :</strong></td><td style='padding: 2px 0;'>" + currentDate + "</td></tr>");
            sb.Append("        </table>");
            sb.Append("      </td>");
            sb.Append("      <td style='vertical-align: top; text-align: right; width: 50%; font-weight: bold; font-size: 13px; line-height: 1.4;'>");
            sb.Append("        <div style='margin-bottom: 2px;'>" + membershipNo + "</div>");
            sb.Append("        <div>" + memberName + "</div>");
            sb.Append("        <div style='font-weight: normal; font-size: 12.5px; margin-top: 4px; line-height: 1.35;'>" + memberAddress + "</div>");
            sb.Append("      </td>");
            sb.Append("    </tr>");
            sb.Append("  </table>");
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 20px;'>Dear Sir/Madam,</div>");
            sb.Append("  <div style='font-size: 13.5px; font-weight: bold; margin-bottom: 20px;'>Subject: <span style='text-decoration: underline;'>RETURN OF THE BOOK(S)</span></div>");
            sb.Append("  <div style='font-size: 13.5px; text-align: justify; margin-bottom: 20px; line-height: 1.6;'>You had drawn the following book(s) on the date(s) shown below from the Club Library. The due date(s) has/have since expired and the book(s) is/are continuously being shown against your name.</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 13px;'>");
            sb.Append("    <thead>");
            sb.Append("      <tr style='border-top: 1.5px solid #000; border-bottom: 1.5px solid #000;'>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 100px;'>Book No</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 120px;'>DDC No</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold;'>Title</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 110px;'>Issue Date</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 110px;'>Due Date</th>");
            sb.Append("      </tr>");
            sb.Append("    </thead>");
            sb.Append("    <tbody>");
            sb.Append("      <tr style='border-bottom: 1.5px solid #000;'>");
            sb.Append("        <td style='padding: 10px 4px; font-family: monospace;'>" + bookNo + "</td>");
            sb.Append("        <td style='padding: 10px 4px;'>" + ddc + "</td>");
            sb.Append("        <td style='padding: 10px 4px; font-weight: 500;'>" + bookTitle + "</td>");
            sb.Append("        <td style='padding: 10px 4px;'>" + issueDate + "</td>");
            sb.Append("        <td style='padding: 10px 4px;'>" + dueDate + "</td>");
            sb.Append("      </tr>");
            sb.Append("    </tbody>");
            sb.Append("  </table>");
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 50px;'>Kindly arrange to return the above mentioned book(s) within \"10\" days of issue of this letter.</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='width: 50%; vertical-align: bottom; font-size: 10px; color: #555; font-style: italic;'>" + auditStamp + "</td>");
            sb.Append("      <td style='width: 50%; text-align: right; vertical-align: top; font-size: 13.5px;'>");
            sb.Append("        <div>Yours Faithfully,</div>");
            sb.Append("        <div style='margin-top: 50px; font-weight: bold;'>Librarian</div>");
            sb.Append("      </td>");
            sb.Append("    </tr>");
            sb.Append("  </table>");
            sb.Append("</div>");
        ...
        {
            string r1Date = row["Reminder1SentDate"] != DBNull.Value 
                ? Convert.ToDateTime(row["Reminder1SentDate"]).ToString("dd/MM/yyyy") 
                : Convert.ToDateTime(row["DueDate"]).AddDays(7).ToString("dd/MM/yyyy");
            ...
            sb.Append("  </table>");
            sb.Append("  <div style='text-align: center; margin: 30px 0 20px 0;'>");
            sb.Append("    <span style='font-size: 20px; font-weight: bold; text-decoration: underline; letter-spacing: 0.5px;'>SECOND REMINDER</span>");
            sb.Append("  </div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 13px;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='vertical-align: top; width: 50%;'>");
            sb.Append("        <table style='border-collapse: collapse;'>");
            sb.Append("          <tr><td style='padding: 2px 0; width: 150px;'><strong>Library Reminder No:</strong></td><td style='padding: 2px 0;'>2-" + string.Format("{0:D6}", loanID) + " / " + membershipNo + "</td></tr>");
            sb.Append("          <tr><td style='padding: 2px 0;'><strong>Dated :</strong></td><td style='padding: 2px 0;'>" + currentDate + "</td></tr>");
            sb.Append("        </table>");
            sb.Append("      </td>");
            sb.Append("      <td style='vertical-align: top; text-align: right; width: 50%; font-weight: bold; font-size: 13px; line-height: 1.4;'>");
            ...
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 20px;'>Dear Sir/Madam,</div>");
            sb.Append("  <div style='font-size: 13.5px; font-weight: bold; margin-bottom: 20px;'>Subject: <span style='text-decoration: underline;'>RETURN OF THE BOOK(S)</span></div>");
            sb.Append("  <div style='font-size: 13.5px; text-align: justify; margin-bottom: 20px; line-height: 1.6;'>You had drawn the following book(s) shown below from the Library.</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 25px; font-size: 12.5px;'>");
            sb.Append("    <thead>");
            sb.Append("      <tr style='border-top: 1.5px solid #000; border-bottom: 1.5px solid #000;'>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 80px;'>Book No</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 100px;'>DDC No</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold;'>Title</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 90px;'>Issue Date</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 90px;'>Due Date</th>");
            sb.Append("        <th style='text-align: left; padding: 8px 4px; font-weight: bold; width: 110px;'>First Reminder Date</th>");
            sb.Append("      </tr>");
            sb.Append("    </thead>");
            sb.Append("    <tbody>");
            sb.Append("      <tr style='border-bottom: 1.5px solid #000;'>");
            ...
            sb.Append("        <td style='padding: 10px 4px;'>" + issueDate + "</td>");
            sb.Append("        <td style='padding: 10px 4px;'>" + dueDate + "</td>");
            sb.Append("        <td style='padding: 10px 4px;'>" + r1Date + "</td>");
            sb.Append("      </tr>");
            sb.Append("    </tbody>");
            sb.Append("  </table>");
            sb.Append("  <div style='font-size: 13.5px; text-align: justify; margin-bottom: 20px; line-height: 1.6;'>On the expiry of the date of return of the book(s) the first reminder was issued as per date mentioned above for return of the same. You have neither deposited the book(s) nor informed the Librarian on the subject.</div>");
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 50px;'>Kindly arrange to return the above mentioned book(s) within \"10\" days of issue of this letter.</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='width: 50%; vertical-align: bottom; font-size: 10px; color: #555; font-style: italic;'>" + auditStamp + "</td>");
        ...
        else if (scenario == 3)
        {
            string r1Date = row["Reminder1SentDate"] != DBNull.Value 
                ? Convert.ToDateTime(row["Reminder1SentDate"]).ToString("dd/MM/yyyy") 
                : Convert.ToDateTime(row["DueDate"]).AddDays(7).ToString("dd/MM/yyyy");
            string r2Date = row["Reminder2SentDate"] != DBNull.Value 
                ? Convert.ToDateTime(row["Reminder2SentDate"]).ToString("dd/MM/yyyy") 
                : Convert.ToDateTime(row["DueDate"]).AddDays(15).ToString("dd/MM/yyyy");

            string currentMonthYear = DateTime.Today.ToString("MM/yyyy");
            string currentOrdinalDate = GetOrdinalDate(DateTime.Today);
            string deadlineOrdinalDate = GetOrdinalDate(DateTime.Today.AddDays(7));
            string phoneStr = row["Phone"] != DBNull.Value && !string.IsNullOrEmpty(row["Phone"].ToString())
                ? " " + row["Phone"].ToString().Trim()
                : "";
            
            string estFineAmt = Convert.ToDecimal(row["EstFine"]).ToString("N0");

            sb.Append("<div style='font-family: Arial, sans-serif; color: #000; line-height: 1.5; padding: 10px; text-align: left;'>");
            sb.Append("  <div style='text-align: center; font-size: 24px; font-weight: bold; letter-spacing: 1.5px; margin-bottom: 25px;'>LAHORE GYMKHANA</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 30px; font-size: 13px;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='width: 40%; vertical-align: top;'></td>");
            sb.Append("      <td style='width: 60%; vertical-align: top; text-align: right; line-height: 1.45;'>");
            sb.Append("        <div style='font-weight: bold; font-family: monospace; font-size: 14px; margin-bottom: 4px;'>LG/LIBRARY/" + membershipNo + "/" + currentMonthYear + "</div>");
            sb.Append("        <div style='margin-bottom: 12px;'>" + currentOrdinalDate + "</div>");
            sb.Append("        <div style='font-weight: bold; margin-bottom: 2px;'>" + membershipNo + "</div>");
            sb.Append("        <div style='font-weight: bold; margin-bottom: 2px;'>" + memberName + "</div>");
            sb.Append("        <div style='font-size: 12.5px; line-height: 1.35;'>" + memberAddress + phoneStr + "</div>");
            sb.Append("      </td>");
            sb.Append("    </tr>");
            sb.Append("  </table>");
            sb.Append("  <div style='text-align: center; margin: 25px 0; font-size: 15px; font-weight: bold;'>Subject: <span style='text-decoration: underline;'>RETURN OF BOOK/S - FINAL REMINDER</span></div>");
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 20px;'>Dear Madam / Sir,</div>");
            sb.Append("  <div style='font-size: 13.5px; text-align: justify; margin-bottom: 20px; line-height: 1.6;'>Please refer to our earlier reminders dated " + r1Date + " and " + r2Date + ". The following book(s) have yet not been returned by your good self:</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse; margin-bottom: 25px; font-size: 13px; border: 1px solid #000;'>");
            sb.Append("    <thead>");
            sb.Append("      <tr>");
            sb.Append("        <th style='border: 1px solid #000; text-align: center; padding: 8px; font-weight: bold; width: 110px; text-transform: uppercase;'>BOOK #</th>");
            sb.Append("        <th style='border: 1px solid #000; text-align: center; padding: 8px; font-weight: bold; text-transform: uppercase;'>TITLE</th>");
            sb.Append("        <th style='border: 1px solid #000; text-align: center; padding: 8px; font-weight: bold; width: 120px; text-transform: uppercase;'>ISSUE DATE</th>");
            sb.Append("        <th style='border: 1px solid #000; text-align: center; padding: 8px; font-weight: bold; width: 120px; text-transform: uppercase;'>DUE DATE</th>");
            sb.Append("      </tr>");
            sb.Append("    </thead>");
            sb.Append("    <tbody>");
            sb.Append("      <tr>");
            sb.Append("        <td style='border: 1px solid #000; padding: 10px; text-align: center; font-family: monospace;'>" + bookNo + "</td>");
            sb.Append("        <td style='border: 1px solid #000; padding: 10px; text-align: left; font-weight: 500;'>" + bookTitle + "</td>");
            sb.Append("        <td style='border: 1px solid #000; padding: 10px; text-align: center;'>" + issueDate + "</td>");
            sb.Append("        <td style='border: 1px solid #000; padding: 10px; text-align: center;'>" + dueDate + "</td>");
            sb.Append("      </tr>");
            sb.Append("    </tbody>");
            sb.Append("  </table>");
            sb.Append("  <div style='font-size: 13.5px; text-align: justify; margin-bottom: 20px; line-height: 1.6;'>You are, therefore, requested to kindly return the book(s) along with a fine of <strong>Rs. " + estFineAmt + "/-</strong> before " + deadlineOrdinalDate + " failing which the Club shall be constrained to charge you the actual price of the book(s) plus 200 % extra charges as per Library Rules to cover replacement cost and other allied expenses, please.</div>");
            sb.Append("  <div style='font-size: 13.5px; margin-bottom: 55px;'>Thanking you.</div>");
            sb.Append("  <table style='width: 100%; border-collapse: collapse;'>");
            sb.Append("    <tr>");
            sb.Append("      <td style='width: 50%; vertical-align: bottom; font-size: 10px; color: #555;'></td>");
            sb.Append("      <td style='width: 50%; text-align: right; vertical-align: top; font-size: 13.5px;'>");
            sb.Append("        <div>Yours sincerely,</div>");
            sb.Append("        <div style='margin-top: 45px; font-weight: bold;'>Secretary</div>");
            sb.Append("      </td>");
            sb.Append("    </tr>");
            sb.Append("  </table>");
            sb.Append("</div>");
    ...
    private void ProcessMarkSent(int loanID)
    {
        DataTable dt = DBHelper.GetOverdueReminderList(null);
        DataRow[] rows = dt.Select("LoanID = " + loanID);
        if (rows.Length > 0)
        {
            ...
            string result = DBHelper.SendOverdueReminder(loanID, scenario);
            if (result.StartsWith("OK"))
            {
                if (scenario == 3)
                {
                    ShowAlert("Final notice sent. Overdue fine, book cost, and 200% penalty have been automatically charged to member " + row["MembershipNo"] + " ledger.", true);
                }
                else
                {
                    ShowAlert("Reminder " + scenario + " successfully sent and registered for member " + row["MembershipNo"] + ".", true);
                }
                BindRemindersGrid();
                BindReversalsGrid();
            }
            ...
            {
                ShowAlert("Failed to process reminder: " + result, false);
            }
        }
    }

    protected void btnCloseLetter_Click(object sender, EventArgs e)
    {
        pnlLetterModal.Visible = false;
    }

    // =========================================================================
    //  REVERSAL DESK ROW COMMANDS (PROCESS REVERSAL & VOUCHER SLIP)
    // =========================================================================
    protected void gvReversals_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "ProcessReversal")
        {
            int loanID = Convert.ToInt32(e.CommandArgument);
            ExecuteReversal(loanID);
        }
    }

    private void ExecuteReversal(int loanID)
    {
        try
        {
            // First look up details before executing reversal (so we can display them in voucher modal)
            DataTable dtCharged = DBHelper.GetFinalChargedLoans();
            DataRow[] rows = dtCharged.Select("LoanID = " + loanID);
            
            if (rows.Length > 0)
            {
                DataRow cRow = rows[0];
                string mNo = cRow["MembershipNo"].ToString();
                string mName = cRow["MemberName"].ToString();
                decimal overdueFine = Convert.ToDecimal(cRow["OverdueFine"]);

                string voucherNo;
                string result = DBHelper.ReverseOverdueCharges(loanID, CurrentStaffID, out voucherNo);

                if (result == "OK")
                {
                    // Bind voucher preview modal
                    litVoucherNo.Text = voucherNo;
                    litVoucherMemberName.Text = mName;
                    litVoucherMemberNo.Text = mNo;
                    litVoucherOverdueFineAmt.Text = overdueFine.ToString("N2");
                    litVoucherNetPayable.Text = overdueFine.ToString("N2");

                    pnlVoucherModal.Visible = true;
                    ShowAlert("Book cost and 200% penalty reversed. Voucher " + voucherNo + " generated for remaining overdue fine (Rs. " + overdueFine.ToString("N2") + ").", true);
                ...
                {
                    ShowAlert("Reversal database error: " + result, false);
                }
            }
            else
            {
                ShowAlert("Error: Charged loan details not found.", false);
            }
        }
        catch (Exception ex)
        {
            ShowAlert("Error processing reversal: " + ex.Message, false);
        }
    }

    protected void btnCloseVoucher_Click(object sender, EventArgs e)
    {
        pnlVoucherModal.Visible = false;
    }

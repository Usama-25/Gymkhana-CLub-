...
public partial class OutgoingClubMemberSearch : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(Request.QueryString["viewreport"]))
        {
            ...
            DataTable dt = Session["OutgoingSearchData"] as DataTable;
            if (dt != null)
            {
                if (type == "list") GenerateListReport(dt, "Outgoing Club Members - Visit Report");
    ...
    protected void gvResults_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("OutgoingClubMembers.aspx?id=" + id);
        }
        else if (e.CommandName == "PrintItem")
        {
            string script = "setTimeout(function() { window.open('OutgoingClubMembers.aspx?print=true&id=" + id + "', '_blank'); }, 100);";
    ...
    {
        if (Session["OutgoingSearchData"] == null)
        {
            DataTable dt = GetData();
            Session["OutgoingSearchData"] = dt;
        }
        string script = "setTimeout(function() { window.open('OutgoingClubMemberSearch.aspx?viewreport=list', '_blank'); }, 100);";
    ...
    protected void btnPrintSummary_Click(object sender, EventArgs e)
    {
        ...
        string script = "setTimeout(function() { window.open('OutgoingClubMemberSearch.aspx?viewreport=summary', '_blank'); }, 100);";
...
<head>
    <meta charset='utf-8'>
    <title>Outgoing Visit Report</title>
    <style>
...
<head>
    <meta charset='utf-8'>
    <title>Outgoing Summary Report</title>
    <style>
        @page { size: A4; margin: 0; }
        body { font-family: 'Arial', sans-serif; font-size: 13px; margin: 0; padding: 0; color: #000; background: #fff; line-height: 1.3; }
        .report-page { width: 210mm; min-height: 297mm; padding: 15mm; margin: auto; box-sizing: border-box; display: flex; flex-direction: column; position: relative; }
...
<body onload='window.print()'>
    <div class='report-page'>
        ...
        <div class='report-title'>Destination Clubwise Visit Summary</div>

        <table>
            <tr>
                <th>Destination Club Name</th>
                <th>Total Visits</th>
            </tr>");

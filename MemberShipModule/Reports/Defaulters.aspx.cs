...
{
    public partial class Defaulters : Page
    {
        ...
        private void BindGrid()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("usp_GetDefaultersReport", conn))
                {
                    ...
                    gvDefaulters.DataSource = dt;
                    gvDefaulters.DataBind();
                }
            }
        }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        th { background: #c53030; color: white; font-weight: 600; padding: 10px 8px; text-align: left; font-size: 10px; }
        td { padding: 8px; border-bottom: 1px solid #e2e8f0; font-size: 10px; }
        tr:nth-child(even) { background: #fff5f5; }
        tr:hover { background: #fed7d7; }
        .member-no { font-weight: 600; color: #2c5282; }
        .status { font-weight: 600; color: #c53030; }
        .badge-yes { background: #c53030; color: white; padding: 2px 8px; border-radius: 4px; font-size: 9px; }
        .badge-no { background: #a0aec0; color: white; padding: 2px 8px; border-radius: 4px; font-size: 9px; }
...
<body>
    <div class='header'>
        ...
        <div class='subtitle'>Suspended / Defaulters Report</div>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>");
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    html.Append("<td><span class='" + (blacklisted ? "badge-yes" : "badge-no") + "'>" + (blacklisted ? "Yes" : "No") + "</span></td>");
                    html.Append("</tr>");
                }
            }
            else
            {
                html.Append("<tr><td colspan='5' class='no-data'>No defaulters found.</td></tr>");
            }
    ...
    <script>window.onload = function() { window.print(); }</script>
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=DefaultersReport.xls");
            Response.Charset = "";
            ...
            using (StringWriter sw = new StringWriter())
            {
                ...
                gvDefaulters.AllowPaging = false;
                this.BindGrid();
                gvDefaulters.RenderControl(hw);

...
public partial class Masters_Site : System.Web.UI.MasterPage
{
    ...
    {
        // ðŸ” Session check (strictly Emp_ID and UserName)
        ...
        // Maintain compatibility with existing library pages that check Session["StaffID"]
        if (Session["StaffID"] == null && Session["Emp_ID"] != null)
        {
            // Failsafe in case StaffID session got lost
            try
            {
                Session["StaffID"] = Convert.ToInt16(Session["Emp_ID"]);
            }
            catch
            {
                Session["StaffID"] = (short)1; // Default fallback
            }
        }

        // ðŸ”¥ Convert Emp_ID properly for DB lookup
        int empId = 0;
        int.TryParse(Session["Emp_ID"].ToString(), out empId);

        // ðŸ”¹ Bind dynamic nav stubs (kept for parity)
        BindDynamicNav();

        // âœ… Load employee/staff details dynamically from database
        if (empId > 0)
        {
            Load_Data(empId);
        }
    ...
    // ===========================
    // ðŸ”¹ LOAD EMPLOYEE DATA FROM USER_MANAGEMENT
    // ===========================
    public void Load_Data(int empId)
    {
        try
        {
            string conString = ConfigurationManager.ConnectionStrings["GymkhanaLibraryDB"].ConnectionString;
                ...
                using (SqlCommand cmd = new SqlCommand(qry, con))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            // If FullName resolved to empty (e.g. both names null), default to UserName session
                            ...
                            lblSubDept.Text = subDeptName;

                            // Populate sidebar/header literal info
                            litStaffName.Text = fullName;
                            litStaffRole.Text = designationName;
        ...
        {
            // Failsafe fallback
            lblEmpName.Text = Session["EFName"] != null ? Session["EFName"].ToString() : "Librarian";
            lblDesignation.Text = Session["StaffRole"] != null ? Session["StaffRole"].ToString() : "Staff";
            lblDepartment.Text = "Library";
            lblSubDept.Text = "Operations";
            divEmpInfo.Visible = true;
        }
    }
    ...
    protected string GetActiveLinkClass(string pageName)
    {
        try
        {
            string currentFile = Path.GetFileName(Request.Url.AbsolutePath);
    ...
    private void BindDynamicNav()
    {
        ...
        try
        {
            string conn = ConfigurationManager.ConnectionStrings["GymkhanaLibraryDB"].ConnectionString;
    ...
    protected void btnLoadMore_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                // Go back to standard 10 modules
                DataTable dtModules = new DataTable();
                using (SqlConnection con = new SqlConnection(conn))
                using (SqlCommand cmd = new SqlCommand("User_management.dbo.usp_MainMenu", con))
                {

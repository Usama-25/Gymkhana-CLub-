...
{
    public partial class DateYearWiseReport : Page
    {
        ...
        {
            if (ddlYear == null) return;
            ddlYear.Items.Clear();
            ddlYear.Items.Add(new ListItem("-- Select All Years --", "0"));
        ...
        private DataTable GetReportData()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("sp_GetDateOrYearWiseReceiptReport", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    DateTime fromDate;
                    if (txtFromDate != null && !string.IsNullOrEmpty(txtFromDate.Text.Trim()) && DateTime.TryParse(txtFromDate.Text.Trim(), out fromDate))
                    {
                    ...
                    DateTime toDate;
                    if (txtToDate != null && !string.IsNullOrEmpty(txtToDate.Text.Trim()) && DateTime.TryParse(txtToDate.Text.Trim(), out toDate))
                    {
                    ...
                    int selectedYear = 0;
                    if (ddlYear != null && ddlYear.SelectedIndex > 0 && int.TryParse(ddlYear.SelectedValue, out selectedYear))
                    {
        ...
        {
            if (txtFromDate != null) txtFromDate.Text = "";
            if (txtToDate != null) txtToDate.Text = "";
            if (ddlYear != null) ddlYear.SelectedIndex = 0;
            if (gvReport != null) gvReport.PageIndex = 0;
            BindReportGrid();
        }
        ...
        protected void btnPrintReport_Click(object sender, EventArgs e)
        {
            ...
            string filterDesc = "All Records";
            if (txtFromDate != null && !string.IsNullOrEmpty(txtFromDate.Text) && txtToDate != null && !string.IsNullOrEmpty(txtToDate.Text))
            {
                filterDesc = string.Format("Date Range: {0:dd-MMM-yyyy} to {1:dd-MMM-yyyy}", DateTime.Parse(txtFromDate.Text), DateTime.Parse(txtToDate.Text));
            }
            else if (ddlYear != null && ddlYear.SelectedIndex > 0)
            {
                filterDesc = "Year: " + ddlYear.SelectedValue;
            }
...
<head>
    <meta charset='utf-8'>
    <title>Date / Year-Wise Receipt Report</title>
    <style>
...
<body>
    <div class='header'>
        ...
        <div class='subtitle'>Date / Year-Wise Receipt & Balance Report</div>
        <div class='meta-info'>Generated On: " + generatedDate + @" | Filter: " + Server.HtmlEncode(filterDesc) + @"</div>
    </div>
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (isFirstForApp)
                    {
                        ...
                        appNameHtml = Server.HtmlEncode(row["ApplicantName"].ToString()) + "<br/><small style='color:#64748b;'>App Date: " + (row["ApplicationDate"] != DBNull.Value ? Convert.ToDateTime(row["ApplicationDate"]).ToString("dd-MMM-yyyy") : "-") + " | CNIC: " + Server.HtmlEncode(row["CNIC"].ToString()) + "</small>";
    ...
    <script>window.onload = function() { window.print(); }</script>
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=DateYearWiseReceiptReport.xls");
            Response.Charset = "";

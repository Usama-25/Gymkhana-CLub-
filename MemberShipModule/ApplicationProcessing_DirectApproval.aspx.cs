...
public partial class ApplicationProcessing_DirectApproval : System.Web.UI.Page
{
    ...
    private void BindAllocatedReceipts()
    {
        ...
        {
            txtTotalPaymentsDisplay.Text = totalPayments.ToString("N0");
        }
    ...
    private void LoadApplicantData(string trackId = null)
    {
        ...
        using (SqlCommand cmd = new SqlCommand("usp_GetApplicantByTrackId", conn))
        {
            ...
            if (dr.Read())
            {
                ...
                SetDropDownListValue(ddlMaritalStatus, dr["MaritalStatus"]);
                txtProfession.Text = dr["Profession"].ToString();
                txtCompanyName.Text = dr["CompanyName"].ToString();
                txtDesignation.Text = dr["Designation"].ToString();
                SetDropDownListValue(ddlNationality, dr["Nationality"]);
                txtMonthlyIncome.Text = dr["MonthlyIncome"].ToString();
                SetDropDownListValue(ddlCurrency, dr["Currency"]);
                ...
                SetDropDownListValue(ddlDegree, dr["Degree"]);
                ...
                SetDropDownListValue(ddlSpouseProfession, dr["SpouseProfession"]);
                SetDropDownListValue(ddlEducation, dr["SpouseEducation"]);
                ...
                btnApproveAndMove.Visible = true; // Added
    ...
    private void ClearFormFields()
    {
        ...
        txtMemberFee.Text = "";
        if (txtTotalPaymentsDisplay != null) txtTotalPaymentsDisplay.Text = "0";
        if (upPaymentSummary != null) upPaymentSummary.Update();
    ...
    protected void btnApproveAndMove_Click(object sender, EventArgs e)
    {
        string nic = txtNIC.Text.Trim();
        if (string.IsNullOrEmpty(nic))
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('NIC is required to proceed.');", true);
            return;
        }

        // Check pending dues against selected Application Form Type & Application Types from FormTable
        ...
        if (pendingDues > 0)
        {
            string formTypeName = (ddlMembershipClass.SelectedIndex > 0 && ddlMembershipClass.SelectedItem != null) ? ddlMembershipClass.SelectedItem.Text : "";
            string subTypeName = (ddlMemberTypes.SelectedIndex > 0 && ddlMemberTypes.SelectedItem != null) ? ddlMemberTypes.SelectedItem.Text : "";

            string alertMsg = string.Format(
                "Cannot proceed with Direct Approval.\\n\\nThis applicant has pending dues of Rs. {0:N0} against the selected Application Form Type ({1}) and Application Type ({2}).\\n\\nTotal Fee Required: Rs. {3:N0}\\nTotal Paid: Rs. {4:N0}\\nPending Dues: Rs. {0:N0}",
                pendingDues,
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "alertDues", "alert('" + alertMsg + "');", true);
            return;
        }
        ...
        using (SqlConnection conn = new SqlConnection(con))
        {
            ...
            {
                cmdCheck.Parameters.Add("@NIC", SqlDbType.NVarChar, 50).Value = nic;
                int count = (int)cmdCheck.ExecuteScalar();
                
                if (count > 0)
                {
                    string updateQuery = "UPDATE maininterview SET Status = 'Approved' WHERE NIC = @NIC";
                    using (SqlCommand cmdUpdate = new SqlCommand(updateQuery, conn))
                    {
                        cmdUpdate.Parameters.Add("@NIC", SqlDbType.NVarChar, 50).Value = nic;
                        cmdUpdate.ExecuteNonQuery();
                    }
                }
                else
                {
                    string insertQuery = "INSERT INTO maininterview (NIC, Status, IName, FatherName, Membership, Email, InterviewId) VALUES (@NIC, 'Approved', @IName, @FatherName, @Membership, @Email, @InterviewId)";
                    using (SqlCommand cmdInsert = new SqlCommand(insertQuery, conn))
                    {
                        cmdInsert.Parameters.Add("@NIC", SqlDbType.NVarChar, 50).Value = nic;
                        cmdInsert.Parameters.Add("@IName", SqlDbType.NVarChar, 200).Value = txtApplicantName.Text.Trim();
                        cmdInsert.Parameters.Add("@FatherName", SqlDbType.NVarChar, 200).Value = txtFatherName.Text.Trim();
                        cmdInsert.Parameters.Add("@Membership", SqlDbType.NVarChar, 100).Value = ddlMembershipClass.SelectedItem != null ? ddlMembershipClass.SelectedItem.Text : "";
                        cmdInsert.Parameters.Add("@Email", SqlDbType.NVarChar, 200).Value = txtEmail.Text.Trim();
                        cmdInsert.Parameters.Add("@InterviewId", SqlDbType.Int).Value = 0; // Dummy ID since we bypass interview
                        cmdInsert.ExecuteNonQuery();
                    }
                }
            }

            // Also update status in ApplicationFForm to Approved
            string updateAppFForm = "UPDATE ApplicationFForm SET Status = 'Approved' WHERE REPLACE(NIC, '-', '') = REPLACE(@NIC, '-', '') OR (TrackID = @TrackID AND @TrackID > 0)";
            using (SqlCommand cmdFForm = new SqlCommand(updateAppFForm, conn))
            {
                cmdFForm.Parameters.Add("@NIC", SqlDbType.NVarChar, 50).Value = nic;
                int tId = 0;
                int.TryParse(txtAppTrackNo.Text.Trim(), out tId);
                cmdFForm.Parameters.Add("@TrackID", SqlDbType.Int).Value = tId;
                cmdFForm.ExecuteNonQuery();
            }
        }

        string url = ResolveUrl("~/MemberShipModule/SearchInterviewResult.aspx?ApplicantID=" + txtAppTrackNo.Text.Trim());
    ...
    private decimal CheckPendingDues(out decimal totalRequired, out decimal totalPaid)
    {
        ...
        if (ddlMembershipClass.SelectedIndex > 0)
        {
            int.TryParse(ddlMembershipClass.SelectedValue, out formTypeMainId);
        }

        int subTypeId = 0;
        if (ddlMemberTypes.SelectedIndex > 0)
        {
            int.TryParse(ddlMemberTypes.SelectedValue, out subTypeId);
        }

        string formTypeMainName = (ddlMembershipClass.SelectedIndex > 0 && ddlMembershipClass.SelectedItem != null)
            ? ddlMembershipClass.SelectedItem.Text.Trim() : "";
        string subTypeName = (ddlMemberTypes.SelectedIndex > 0 && ddlMemberTypes.SelectedItem != null)
            ? ddlMemberTypes.SelectedItem.Text.Trim() : "";
        ...
        using (SqlConnection conn = new SqlConnection(con))
        {
            ...
            string query = @"
                ...
                WHERE 
                    (
                        (@FormTypeMainId > 0 AND FormTypeMainId = @FormTypeMainId AND (@SubTypeId = 0 OR SubTypeId = @SubTypeId OR SubTypeId IS NULL))
                        OR 
                        (@FormTypeMainName <> '' AND (FormTypeName = @FormTypeMainName OR FormTypeName = @SubTypeName))
                    )
                ORDER BY 
                    CASE WHEN @FormTypeMainId > 0 AND FormTypeMainId = @FormTypeMainId AND @SubTypeId > 0 AND SubTypeId = @SubTypeId THEN 0 
                         WHEN @FormTypeMainId > 0 AND FormTypeMainId = @FormTypeMainId THEN 1 
                         ELSE 2 END,
            ...
            {
                cmd.Parameters.Add("@FormTypeMainId", SqlDbType.Int).Value = formTypeMainId;
                cmd.Parameters.Add("@SubTypeId", SqlDbType.Int).Value = subTypeId;
        ...
        if (dtReceipts != null)
        {
            foreach (DataRow row in dtReceipts.Rows)
            {
                if (row["Amount"] != DBNull.Value)
                {
                    ...
                    if (decimal.TryParse(row["Amount"].ToString().Replace(",", ""), out amt))
                    {
                        paidFromReceipts += amt;
                    }
                }
            }
        }

        decimal formFeeTxt = 0;
        decimal memberFeeTxt = 0;
        decimal.TryParse(txtFormFee.Text.Trim().Replace(",", ""), out formFeeTxt);
        decimal.TryParse(txtMemberFee.Text.Trim().Replace(",", ""), out memberFeeTxt);
        decimal paidFromText = formFeeTxt + memberFeeTxt;

        totalPaid = Math.Max(paidFromReceipts, paidFromText);
    ...
    private void AddCommonParameters(SqlCommand cmd, Dictionary<string, string> filePaths, int trackId)
    {
        ...
        if (ddlMemberTypes.SelectedIndex > 0 && int.TryParse(ddlMemberTypes.SelectedValue, out memberShipTypeId))
        {
            cmd.Parameters.Add("@MemberShipTypeID", SqlDbType.Int).Value = memberShipTypeId;
        }
        else
        {
            cmd.Parameters.Add("@MemberShipTypeID", SqlDbType.Int).Value = DBNull.Value;
        }
    ...
    private void BindChildrenGrid()
    {
        ...
        {
            if (!string.IsNullOrEmpty(child.Relationship))
            {
                if (child.Relationship.Equals("Son", StringComparison.OrdinalIgnoreCase)) sons++;
                else if (child.Relationship.Equals("Daughter", StringComparison.OrdinalIgnoreCase)) daughters++;
            }
        }
    ...
    protected void txtChildDOB_TextChanged(object sender, EventArgs e)
    {
        ...
        DateTime dob;
        if (DateTime.TryParseExact(txtDOBChild.Text, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out dob) || DateTime.TryParse(txtDOBChild.Text, out dob))
        {
    ...
    private void UpdateChildrenListFromGrid()
    {
        ...
        for (int i = 0; i < gvChildren.Rows.Count; i++)
        {
            ...
            if (row.RowType == DataControlRowType.DataRow)
            {
                ...
                DateTime d;
                if (DateTime.TryParseExact(txtDOBChild.Text, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out d) || DateTime.TryParse(txtDOBChild.Text, out d))
                {
                    item.DOB = d;
    ...
    private void SetDropDownListValue(DropDownList ddl, object val)
    {
        if (ddl == null) return;
        ddl.ClearSelection();
        if (val != null && val != DBNull.Value)
        {
            ...
            ListItem itemByVal = ddl.Items.FindByValue(strVal);
            if (itemByVal != null)
            {
                itemByVal.Selected = true;
                return;
            }

            ListItem itemByText = ddl.Items.FindByText(strVal);
            if (itemByText != null)
            {
                itemByText.Selected = true;
                return;
            }
        }
    }

...
-- 1. Create BankCards table
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'BankCards')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('MemberSubscriptions') AND name = 'BankID')
BEGIN
    ALTER TABLE MemberSubscriptions ADD BankID INT NULL;
    ALTER TABLE MemberSubscriptions ADD BankDiscount DECIMAL(18,2) NOT NULL DEFAULT 0;
END
GO

-- 3. Add Columns to POSTransactions
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('POSTransactions') AND name = 'BankID')
BEGIN
    ALTER TABLE POSTransactions ADD BankID INT NULL;
    ALTER TABLE POSTransactions ADD BankDiscount DECIMAL(18,2) NOT NULL DEFAULT 0;
END
GO

-- 4. Create SPs for BankCards
CREATE OR ALTER PROCEDURE sp_GetBankCards
    @IncludeInactive BIT = 0
AS
BEGIN
    SET NOCOUNT ON;
    IF @IncludeInactive = 1
        SELECT BankID, BankName, DiscountPercentage, IsActive FROM BankCards ORDER BY BankName;
    ELSE
        SELECT BankID, BankName, DiscountPercentage, IsActive FROM BankCards WHERE IsActive = 1 ORDER BY BankName;
END
GO

CREATE OR ALTER PROCEDURE sp_InsertBankCard
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO BankCards (BankName, DiscountPercentage, IsActive)
    VALUES (@BankName, @DiscountPercentage, @IsActive);
END
GO

CREATE OR ALTER PROCEDURE sp_UpdateBankCard
    @BankID INT,
    @BankName NVARCHAR(100),
    @DiscountPercentage DECIMAL(5,2),
    @IsActive BIT

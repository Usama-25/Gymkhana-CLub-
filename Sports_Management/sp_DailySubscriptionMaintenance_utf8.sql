Text                                                                                                                                                                                                                                                           
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- 3. Alter sp_DailySubscriptionMaintenance
CREATE PROCEDURE sp_DailySubscriptionMaintenance
AS
BEGIN
    SET NOCOUNT ON;
...
WHERE CAST(TransactionDate AS DATE) < @CurrentDate
    ...
    DECLARE 
@DependentMemberNo NVARCHAR(50), @DependentName NVARCHAR(150), @DependentRelation NVARCHAR(50);
    DECLARE @PolicyDiscount DECIMAL(18,2), @ManualDiscount DECIMAL(18,2), @NetFee DECIMAL(18,2);
    
    DECLARE curContinuous CURSOR FOR
    SELECT ms.Member
SubID, ms.MemberID, ms.SubscriptionID, ms.LastBilledDate,
    ...
    INNER J
OIN Subscriptions s ON ms.SubscriptionID = s.SubscriptionID
    WHERE ms.IsActive = 1
      AND ms.EndDate IS NULL
      AND DATEADD(month, 1, ms.LastBilledDate) <= @CurrentDate;

    OPEN curContinuous;
    FETCH NEXT FROM curContinuous INTO @MemberSubID
, @MemberID, @SubscriptionID, @LastBilledDate,
   ...
   BEGIN
        ...
        SELECT @BaseFee = Fee, @GSTPercent = ISNULL(GSTPercentage, 16), @PackageName = PackageName FROM Su
bscriptions WHERE SubscriptionID = @SubscriptionID;

        -- Dynamically calculate the new fee based on current BaseFee and current GST, but keep original discounts
        DECLARE @CalculatedFee DECIMAL(18,2);
        
        -- Policy discount might
 have been calculated as 50% or 100% of the OLD base fee. 
        -- If we dynamically change the base fee, the old fixed discount amount might be incorrect.
        -- Therefore, we either stick to NetFee, or we assume discounts are fixed.
        -- Le
t's stick to the dynamically calculated fee:
        
        DECLARE @FeeAfterPolicy DECIMAL(18,2) = @BaseFee - @PolicyDiscount;
        IF @FeeAfterPolicy < 0 SET @FeeAfterPolicy = 0;
        
        DECLARE @GSTAmount DECIMAL(18,2) = @FeeAfterPolicy *
 (@GSTPercent / 100.0);
        
        SET @CalculatedFee = @FeeAfterPolicy + @GSTAmount - @ManualDiscount;
        IF @CalculatedFee < 0 SET @CalculatedFee = 0;
        
        -- If NetFee is not null, use calculated fee (in case GST changed), otherw
ise fallback to BaseFee

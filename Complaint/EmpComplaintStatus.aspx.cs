...
{
    public partial class Pages_EmpComplaintStatus : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    // Session present â€” show locked identity badge, hide dropdown
                    pnlSessionEmployee.Visible = true;
                    pnlDropdownEmployee.Visible = false;

                    // Resolve employee name: prefer session StaffName / EFName, else look it up
                ...
                {
                    // No session â€” show dropdown, hide badge
                    pnlSessionEmployee.Visible = false;
                    pnlDropdownEmployee.Visible = true;
                    BindEmployees();

                    gvComplaints.DataSource = new DataTable();
        ...
        protected void ddlSenderEmployee_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindComplaints();
        }
        ...
        {
            // Prefer session EmpID; fall back to dropdown selection
            decimal empId;
            if (Session["Emp_ID"] != null)
            {
                empId = (Session["Emp_ID"] != null ? Convert.ToDecimal(Session["Emp_ID"]) : 1m);
            }
            else
            {
                ...
                empId = Convert.ToDecimal(ddlSenderEmployee.SelectedValue);
            }

            try
            {
                // Fetch all complaints and filter by the employee in memory
                DataTable dt = DBHelper.GetEmployeeComplaints(null, null, null, null, null, null);
                
                DataView dv = dt.DefaultView;
                dv.RowFilter = "SenderEmpID = " + empId;
            ...
            {
                ShowAlert("Error loading your complaints: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void gvComplaints_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "SendReminder")
            {
                try
                {
                    ...
                    string result = DBHelper.SendEmployeeComplaintReminder(complaintID);
                    
                    if (result.Contains("successfully"))
                    {
                        ShowAlert(result, "alert-success");
                        BindComplaints(); // Refresh count and date
                    }
                ...
                {
                    ShowAlert("Error sending reminder: " + ex.Message, "alert-error");
                }
            }
            else if (e.CommandName == "CloseComplaint")
            {
                try
                {
                    int complaintID = Convert.ToInt32(e.CommandArgument);
                    string result = DBHelper.UpdateEmployeeComplaintStatus(complaintID, "Closed", "Closed by registering employee.");
                    
                    if (result.Contains("successfully"))
                    {
                        ShowAlert("Complaint closed successfully.", "alert-success");
                        BindComplaints();
                    }
                    else
                    {
                ...
                {
                    ShowAlert("Error closing complaint: " + ex.Message, "alert-error");
                }
            }
        }

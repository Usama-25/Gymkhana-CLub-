...
namespace GymkhanaLibrary
{
    ...
    public partial class Pages_DefineFeedback : System.Web.UI.Page
    {
        ...
        private void BindDepartments()
        {
            ...
            {
                ShowAlert("Error loading departments: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void ddlDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlDepartment.SelectedValue != "0")
            {
                ...
                BindSubDepartments(deptId);
                BindLocations(deptId, null);
            }
            else
            {
        ...
        protected void ddlSubDepartment_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlDepartment.SelectedValue != "0")
            {
                int deptID = Convert.ToInt32(ddlDepartment.SelectedValue);
                int? subDeptID = null;
                if (ddlSubDepartment.SelectedValue != "0" && !string.IsNullOrEmpty(ddlSubDepartment.SelectedValue))
                {
                    subDeptID = Convert.ToInt32(ddlSubDepartment.SelectedValue);
                }
                BindLocations(deptID, subDeptID);
            }
        }

        private void BindSubDepartments(int deptId)
        {
            try
            {
                DataTable dt = DBHelper.GetSubDepartments(deptId);
                ddlSubDepartment.Items.Clear();
                if (dt.Rows.Count > 0)
                {
                    ...
                    ddlSubDepartment.Items.Insert(0, new ListItem("- Select Subdepartment -", "0"));
                    ddlSubDepartment.Enabled = true;
                }
                else
                {
                    ddlSubDepartment.Items.Insert(0, new ListItem("- No Subdepartments -", "0"));
                    ddlSubDepartment.Enabled = false;
                }
            }
            catch (Exception ex)
        ...
        private void BindLocations(int deptId, int? subDeptId)
        {
            try
            {
                DataTable dt = DBHelper.GetFeedbackLocations(deptId, subDeptId);
                ddlLocation.Items.Clear();
                if (dt.Rows.Count > 0)
                {
                ...
                {
                    ddlLocation.Items.Insert(0, new ListItem("- No Locations -", "0"));
                    ddlLocation.Enabled = false;
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading locations: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void btnAddLocation_Click(object sender, EventArgs e)
        {
            if (ddlDepartment.SelectedValue == "0")
            {
                ShowAlert("Please select a department first.", "alert-error");
                return;
            }

            txtModalDept.Text = ddlDepartment.SelectedItem.Text;
            txtModalSubDept.Text = ddlSubDepartment.Enabled && ddlSubDepartment.SelectedValue != "0" 
                ? ddlSubDepartment.SelectedItem.Text 
                : "- No Subdepartment -";
            txtModalLocationName.Text = "";
            pnlLocationModal.Visible = true;
        }
        ...
        protected void btnSaveLocation_Click(object sender, EventArgs e)
        {
            ...
            string locName = txtModalLocationName.Text.Trim();
            ...
            try
            {
                int deptID = Convert.ToInt32(ddlDepartment.SelectedValue);
                int? subDeptID = null;
                if (ddlSubDepartment.Enabled && ddlSubDepartment.SelectedValue != "0" && !string.IsNullOrEmpty(ddlSubDepartment.SelectedValue))
                {
                    subDeptID = Convert.ToInt32(ddlSubDepartment.SelectedValue);
                }

                string msg = DBHelper.SaveFeedbackLocation(deptID, subDeptID, locName);
                if (msg.Contains("successfully"))
                {
                    ShowAlert(msg, "alert-success");
                    pnlLocationModal.Visible = false;
                    
                    // Rebind locations and select the newly created one
                    BindLocations(deptID, subDeptID);
                    ListItem item = ddlLocation.Items.FindByText(locName);
                    if (item != null)
                    {
                        ddlLocation.SelectedValue = item.Value;
                    }
                }
                else
                {
            ...
            {
                ShowAlert("Error saving location: " + ex.Message, "alert-error");
            }
        }
        ...
        private void BindQuestions()
        {
            try
            {
                DataTable dt = DBHelper.GetAllFeedbackQuestions();
                ...
                if (!string.IsNullOrEmpty(search))
                {
                    ...
                    dv.RowFilter = string.Format("QuestionText LIKE '%{0}%' OR DepartmentName LIKE '%{0}%' OR ISNULL(SubDepartmentName, '') LIKE '%{0}%' OR ISNULL(LocationName, '') LIKE '%{0}%'", search);
                }

                DataTable filteredDt = dv.ToTable();
                var groupedList = new List<SubDeptGroup>();

                if (filteredDt.Rows.Count > 0)
                {
                    var groups = filteredDt.AsEnumerable()
                        .GroupBy(r => new {
                            DeptID = r.Field<int>("DeptID"),
                            SubDeptID = r.Field<int?>("SubDeptID"),
                            LocationID = r.Field<int?>("LocationID"),
                            DepartmentName = r.Field<string>("DepartmentName"),
                            SubDepartmentName = r.Field<string>("SubDepartmentName"),
                            LocationName = r.Field<string>("LocationName")
                        });

                    foreach (var g in groups)
                    {
                        DataTable groupTable = filteredDt.Clone();
                        foreach (var row in g)
                        {
                            groupTable.ImportRow(row);
                        }

                        groupedList.Add(new SubDeptGroup
                        {
                            DeptID = g.Key.DeptID,
                            SubDeptID = g.Key.SubDeptID,
                            LocationID = g.Key.LocationID,
                            DepartmentName = g.Key.DepartmentName,
                            SubDepartmentName = g.Key.SubDepartmentName,
                            LocationName = g.Key.LocationName,
                            QuestionsTable = groupTable
                        });
                    }
                }

                rptSubDeptGroups.DataSource = groupedList;
                rptSubDeptGroups.DataBind();

                if (pnlNoQuestions != null)
                {
                    pnlNoQuestions.Visible = (groupedList.Count == 0);
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading questions: " + ex.Message, "alert-error");
            }
        }

        protected void rptSubDeptGroups_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                SubDeptGroup group = (SubDeptGroup)e.Item.DataItem;
                GridView gvSubQuestions = (GridView)e.Item.FindControl("gvSubQuestions");
                if (gvSubQuestions != null && group != null)
                {
                    gvSubQuestions.DataSource = group.QuestionsTable;
        ...
        {
            pnlOptions.Visible = (ddlQuestionType.SelectedValue == "Multiple Choice");
        }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (ddlDepartment.SelectedValue == "0")
            {
                ShowAlert("Please select a department.", "alert-error");
                return;
            }
            ...
            {
                ShowAlert("Please enter a question text.", "alert-error");
                return;
            }
            ...
            if (qType == "Multiple Choice")
            {
                ...
                {
                    ShowAlert("Please enter options separated by comma for Multiple Choice question.", "alert-error");
                    return;
                }

                string[] opts = options.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                if (opts.Length < 2)
                {
                    ShowAlert("Please enter at least two multiple choice options separated by comma (e.g. Excellent, Good, Poor) before saving.", "alert-error");
                    return;
                }
            }
            ...
            try
            {
                ...
                {
                    qID = Convert.ToInt32(hfQuestionID.Value);
                }
                ...
                int? locationID = null;
                if (ddlLocation.Enabled && ddlLocation.SelectedValue != "0" && !string.IsNullOrEmpty(ddlLocation.SelectedValue))
                {
                ...
                string result = DBHelper.SaveFeedbackQuestion(qID, deptID, subDeptID, locationID, qText, qType, options, isActive);
            ...
            {
                ShowAlert("Error saving feedback question: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void gvSubQuestions_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "EditQuestion")
            {
                try
                {
                    ...
                    DataTable dt = DBHelper.GetAllFeedbackQuestions();
                    DataRow[] rows = dt.Select("QuestionID = " + qID);
                    if (rows.Length > 0)
                    {
                        DataRow r = rows[0];
                        hfQuestionID.Value = r["QuestionID"].ToString();
                        
                        string deptID = r["DeptID"].ToString();
                        if (ddlDepartment.Items.FindByValue(deptID) != null)
                        {
                            ddlDepartment.SelectedValue = deptID;
                            BindSubDepartments(Convert.ToInt32(deptID));
                            
                            string subDeptID = r["SubDeptID"] == DBNull.Value ? "0" : r["SubDeptID"].ToString();
                            if (ddlSubDepartment.Enabled && ddlSubDepartment.Items.FindByValue(subDeptID) != null)
                            {
                                ddlSubDepartment.SelectedValue = subDeptID;
                            }

                            // Load locations and bind
                            int? sDeptId = null;
                            if (subDeptID != "0") sDeptId = Convert.ToInt32(subDeptID);
                            BindLocations(Convert.ToInt32(deptID), sDeptId);

                            string locationID = r["LocationID"] == DBNull.Value ? "0" : r["LocationID"].ToString();
                            if (ddlLocation.Enabled && ddlLocation.Items.FindByValue(locationID) != null)
                            {
                                ddlLocation.SelectedValue = locationID;
                            }
                        }

                        txtQuestionText.Text = r["QuestionText"].ToString();
                        
                        string qType = (r["QuestionType"] != DBNull.Value && !string.IsNullOrEmpty(r["QuestionType"].ToString())) ? r["QuestionType"].ToString() : "Rating";
                        if (ddlQuestionType.Items.FindByValue(qType) != null)
                        {
                            ddlQuestionType.SelectedValue = qType;
                        }
                        else
                        {
                            ddlQuestionType.SelectedValue = "Rating";
                        }

                        string opts = r["Options"] != DBNull.Value ? r["Options"].ToString() : "";
                        txtOptions.Text = opts;
                        pnlOptions.Visible = (ddlQuestionType.SelectedValue == "Multiple Choice");

                        chkActive.Checked = Convert.ToBoolean(r["IsActive"]);

                        litFormTitle.Text = "Edit Feedback Question";
                        btnClear.Visible = true;
                    }
                }
                catch (Exception ex)
                {
                    ShowAlert("Error loading question details: " + ex.Message, "alert-error");
                }
            }
        }

        public string GetQrCodeUrl(object deptIDObj, object subDeptIDObj, object locationIDObj = null)
        {
            if (deptIDObj == null || deptIDObj == DBNull.Value) return "";
            string deptID = deptIDObj.ToString();

            string authority = Request.Url.GetLeftPart(UriPartial.Authority);

            string baseUrl; // Full base including authority + virtual path

            if (authority.Contains("192.168.12.40"))
            {
                baseUrl = "http://192.168.12.40/Gymkhana/Complaint/";
            }
            else if (authority.Contains("110.38.232.58"))
            {
                // Hardcode full public URL with port for public IP access
                baseUrl = "http://110.38.232.58:7400/Gymkhana/Complaint/";
            }
            else if (authority.Contains("localhost") || authority.Contains("127.0.0.1"))
            {
                string appPath = Request.ApplicationPath;
                string path = appPath.EndsWith("/") ? appPath : appPath + "/";
                if (!path.Contains("Complaint/"))
                    path += "Complaint/";
                baseUrl = authority + path;
            }
            else
            {
            ...
            string targetUrl = baseUrl + "FeedbackSubmit.aspx?DeptId=" + deptID;

            if (subDeptIDObj != null && subDeptIDObj != DBNull.Value && subDeptIDObj.ToString() != "0")
            {
                targetUrl += "&SubDeptId=" + subDeptIDObj.ToString();
            }

            if (locationIDObj != null && locationIDObj != DBNull.Value && locationIDObj.ToString() != "0" && locationIDObj.ToString() != "")
            {
                targetUrl += "&LocationId=" + locationIDObj.ToString();
            }

            return "https://api.qrserver.com/v1/create-qr-code/?size=400x400&ecc=H&data=" + Server.UrlEncode(targetUrl);
        }

        public string GetPrintSubtitle(object departmentNameObj, object subDepartmentNameObj, object locationNameObj = null)
        {
            string subDept = (subDepartmentNameObj != null && subDepartmentNameObj != DBNull.Value) ? subDepartmentNameObj.ToString().Trim() : "";
            string location = (locationNameObj != null && locationNameObj != DBNull.Value) ? locationNameObj.ToString().Trim() : "";

            if (location.Equals("Entire Dept", System.StringComparison.OrdinalIgnoreCase))
                location = "";

            if (!string.IsNullOrEmpty(subDept) && !string.IsNullOrEmpty(location))
            {
                // Normalize for comparison
                string normSub = System.Text.RegularExpressions.Regex.Replace(subDept.ToLower(), @"[^a-z0-9]", "");
                string normLoc = System.Text.RegularExpressions.Regex.Replace(location.ToLower(), @"[^a-z0-9]", "");

                // Prefix prefix check (e.g. cafe vs cafee)
                string prefixSub = normSub.Length >= 4 ? normSub.Substring(0, 4) : normSub;
                if (!string.IsNullOrEmpty(prefixSub) && normLoc.StartsWith(prefixSub))
                {
                    return location.Replace("\"", "\\\"");
                }
                if (normLoc.Contains(normSub) || normSub.Contains(normLoc))
                {
                    return location.Replace("\"", "\\\"");
                }
                return (subDept + " - " + location).Replace("\"", "\\\"");
            }
            if (!string.IsNullOrEmpty(location))
                return location.Replace("\"", "\\\"");

            return subDept.Replace("\"", "\\\"");
        }

        // Returns general text for the standee title.
        public string GetStandeeTitle(object departmentNameObj, object subDepartmentNameObj, object locationNameObj = null)
        {
            return "FEEDBACK / SUGGESTIONS / COMPLAINTS";
        }

        public string GetPrintClickEvent(object departmentNameObj, object subDepartmentNameObj, object locationNameObj, object deptIDObj, object subDeptIDObj, object locationIDObj)
        {
            string title = GetStandeeTitle(departmentNameObj, subDepartmentNameObj, locationNameObj);
            string subDeptLoc = GetPrintSubtitle(departmentNameObj, subDepartmentNameObj, locationNameObj);
            string qrUrl = GetQrCodeUrl(deptIDObj, subDeptIDObj, locationIDObj);
            return "printQrCard(\"" + title + "\", \"" + subDeptLoc + "\", \"" + qrUrl + "\")";
        }
        ...
        private void ResetForm()
        {
            ...
            litFormTitle.Text = "Define Feedback Question";
...
public static class DBHelper
{
    ...
    public static DataTable GetFeedbackLocations(int deptID, int? subDeptID)
    {
        var prms = new[]
        {
            new SqlParameter("@DeptID", deptID),
        ...
        return ExecuteReader("dbo.sp_GetFeedbackLocations", prms);
    }

    public static string SaveFeedbackLocation(int deptID, int? subDeptID, string locationName)
    {
        var prms = new[]
        {
            new SqlParameter("@DeptID", deptID),
        ...
        ExecuteNonQuery("dbo.sp_SaveFeedbackLocation", prms);
        return GetOutputValue<string>(prms, "@Msg");
    }
    ...
    public static string SaveFeedbackQuestion(int? questionID, int deptID, int? subDeptID, int? locationID, string text, string questionType, string options, bool isActive)
    {
        var prms = new[]
        {
            ...
            new SqlParameter("@QuestionType", (object)questionType ?? "Rating"),
            new SqlParameter("@Options",      (object)options ?? DBNull.Value),
            new SqlParameter("@IsActive",      isActive),
            new SqlParameter("@Msg",           SqlDbType.VarChar, 200) { Direction = ParameterDirection.Output }
        };
...
/// <summary>
/// Email utility for Lahore Gymkhana Library.
/// Uses the complaint@lahoregymkhana.org.pk Gmail account via SMTP.
/// Webmail: https://lahoregymkhana.org.pk/webmail
/// </summary>
public static class EmailHelper
{
    // ── Webmail SMTP credentials ────────────────────────────────────
    ...
    private const string FromName     = "Lahore Gymkhana Library";

    /// <summary>
    /// Send a plain-text or HTML email to one recipient.
    /// </summary>
    /// <param name="toAddress">Recipient email address (member's ResidentialEmail).</param>
    /// <param name="toName">Recipient display name.</param>
    /// <param name="subject">Email subject line.</param>
    /// <param name="body">Email body — may contain HTML.</param>
    /// <param name="isHtml">Set true when body contains HTML markup.</param>
    /// <returns>Empty string on success; error message on failure.</returns>
    public static string SendEmail(string toAddress, string toName, string subject, string body, bool isHtml = true)
    {
        try
        {
            ...
            {
                msg.From = new System.Net.Mail.MailAddress(SmtpUser, FromName);
                msg.To.Add(new System.Net.Mail.MailAddress(toAddress.Trim(), toName));
                msg.Subject    = subject;
                msg.Body       = body;
                ...
                {
                    smtp.EnableSsl   = true;   // port 465 — implicit SSL
    ...
    /// <summary>
    /// Convenience overload — send to multiple recipients.
    /// </summary>
    public static string SendEmail(IEnumerable<string> toAddresses, string subject, string body, bool isHtml = true)
    {
        try
        {
            using (var msg = new System.Net.Mail.MailMessage())
            {
                ...
                if (msg.To.Count == 0) return "No valid recipients.";
    ...
    /// <summary>
    /// Send an overdue book reminder email to a library member.
    /// </summary>
    public static string SendOverdueReminderEmail(string memberEmail, string memberName,
        string bookTitle, DateTime dueDate, int overdueDays, decimal fineAmount)
    {
        string subject = "Library Overdue Notice – " + bookTitle;
        string body = string.Format(@"
<html><body style='font-family:Calibri,sans-serif;color:#1e293b;'>
  <p>Dear <strong>{0}</strong>,</p>
  <p>This is a reminder from the <strong>Lahore Gymkhana Library</strong> that the following book is overdue:</p>
  <table style='border-collapse:collapse;width:100%;max-width:480px;'>
    <tr><td style='padding:6px 12px;font-weight:600;background:#f8fafc;'>Book</td>
        <td style='padding:6px 12px;border:1px solid #e2e8f0;'>{1}</td></tr>
    <tr><td style='padding:6px 12px;font-weight:600;background:#f8fafc;'>Due Date</td>
        <td style='padding:6px 12px;border:1px solid #e2e8f0;'>{2:dd-MMM-yyyy}</td></tr>
    <tr><td style='padding:6px 12px;font-weight:600;background:#f8fafc;'>Days Overdue</td>
        <td style='padding:6px 12px;border:1px solid #e2e8f0;color:#dc2626;font-weight:700;'>{3} day(s)</td></tr>
    <tr><td style='padding:6px 12px;font-weight:600;background:#f8fafc;'>Fine Accrued</td>
        <td style='padding:6px 12px;border:1px solid #e2e8f0;color:#dc2626;font-weight:700;'>Rs. {4:N2}</td></tr>
  </table>
  <p>Please return the book at your earliest convenience to avoid further charges.</p>
  <p style='margin-top:24px;font-size:12px;color:#94a3b8;'>
    Lahore Gymkhana Library &mdash; <a href='https://lahoregymkhana.org.pk/webmail'>lahoregymkhana.org.pk/webmail</a>
  </p>
</body></html>", memberName, bookTitle, dueDate, overdueDays, fineAmount);

        return SendEmail(memberEmail, memberName, subject, body, isHtml: true);
    }

    // ── Feedback confirmation email ──────────────────────────────────────────
    /// <summary>
    /// Send a thank-you confirmation after a member submits feedback.
    /// </summary>
    public static string SendFeedbackConfirmationEmail(string memberEmail, string memberName, string departmentName)
    {
        string subject = "Thank you for your feedback – Lahore Gymkhana";
        string body = string.Format(@"
<html><body style='font-family:Calibri,sans-serif;color:#1e293b;'>
  <p>Dear <strong>{0}</strong>,</p>
  <p>Thank you for taking the time to share your feedback regarding <strong>{1}</strong>.</p>
  <p>Your input helps us continually improve our services at Lahore Gymkhana.</p>
  <p style='margin-top:24px;font-size:12px;color:#94a3b8;'>
    Lahore Gymkhana &mdash; <a href='https://lahoregymkhana.org.pk/webmail'>lahoregymkhana.org.pk/webmail</a>
  </p>
</body></html>", memberName, departmentName);

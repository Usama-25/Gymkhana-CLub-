...
{
    public partial class Pages_EmpComplaintSubmit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    // Session present Ã¢â‚¬â€ show locked badge, hide dropdown, skip DB call
                    pnlSessionEmployee.Visible = true;
                    pnlDropdownEmployee.Visible = false;

                    string empName = Session["EFName"] != null
                        ? Session["EFName"].ToString()
                        : "Employee #" + Session["Emp_ID"].ToString();
                    litEmployeeName.Text = System.Web.HttpUtility.HtmlEncode(empName);
                }
                else
                {
                    // No session Ã¢â‚¬â€ show dropdown, hide badge
                    pnlSessionEmployee.Visible = false;
                    pnlDropdownEmployee.Visible = true;
                    BindEmployees();
                }
            }
        }
        ...
        private void BindEmployees()
        {
            try
            {
                ...
                ddlSenderEmployee.DataSource = dt;
                ddlSenderEmployee.DataTextField = "EmployeeName";
                ddlSenderEmployee.DataValueField = "EmpID";
                ddlSenderEmployee.DataBind();
                ddlSenderEmployee.Items.Insert(0, new ListItem("- Select Your Name -", "0"));
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading employees list: " + ex.Message, "alert-error");
            }
        }
        ...
        private void BindDepartments()
        {
            try
            {
                ...
                ddlTargetDept.DataSource = dt;
                ddlTargetDept.DataTextField = "SubDept_Name";
                ddlTargetDept.DataValueField = "SubDept_Id";
                ddlTargetDept.DataBind();
                ddlTargetDept.Items.Insert(0, new ListItem("- Select Target Subdepartment -", "0"));
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading target subdepartments: " + ex.Message, "alert-error");
            }
        }

        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            // If session has EmpID, use it; otherwise fall back to the dropdown selection
            decimal senderEmpID;
            if (Session["Emp_ID"] != null)
            {
                senderEmpID = (Session["Emp_ID"] != null ? Convert.ToDecimal(Session["Emp_ID"]) : 1m);
            }
            else
            {
                if (ddlSenderEmployee.SelectedValue == "0")
                {
                    ShowAlert("Please select your employee identity.", "alert-error");
                    return;
                }
                senderEmpID = Convert.ToDecimal(ddlSenderEmployee.SelectedValue);
            }

            if (ddlTargetDept.SelectedValue == "0")
            {
                ShowAlert("Please select a target subdepartment.", "alert-error");
                return;
            }
            ...
            {
                ShowAlert("Please enter a complaint subject.", "alert-error");
                return;
            }
            ...
            {
                ShowAlert("Please enter complaint details.", "alert-error");
                return;
            }

            try
            {
                int targetSubDeptID = Convert.ToInt32(ddlTargetDept.SelectedValue);
                int targetDeptID = DBHelper.GetDeptIdBySubDeptId(targetSubDeptID);

                string result = DBHelper.SubmitEmployeeComplaint(senderEmpID, targetDeptID, targetSubDeptID, subject, detail);
            ...
            {
                ShowAlert("Error registering complaint: " + ex.Message, "alert-error");
            }
        }

        private void ResetForm()
        {
            if (ddlSenderEmployee.Items.Count > 0)
            {
                ddlSenderEmployee.SelectedIndex = 0;
            }
            if (ddlTargetDept.Items.Count > 0)
            {
                ddlTargetDept.SelectedIndex = 0;
            }
            txtSubject.Text = "";
            txtDetail.Text = "";
        }

...
{
    public partial class Pages_FeedbackSubmit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // Toggle is permanently locked ON — enforce on every request (initial load AND postback),
            // so it cannot be forced off via client-side tampering (dev tools, direct POST, etc.)
            chkIsComplaint.Checked = true;
            pnlRating.Visible = false;
            pnlSubject.Visible = true;
            btnSubmit.Text = "Submit Complaint";
            litCommentsLabel.Text = "Complaint Details";

            if (!IsPostBack)
            {
                ResolveAndLoadTarget();
            }
        }

        private void ResolveAndLoadTarget()
        {
            string deptIdStr = Request.QueryString["DeptId"];
            if (string.IsNullOrEmpty(deptIdStr))
            {
                ShowAlert("Invalid feedback URL. Department ID parameter missing.", "alert-error");
                btnSubmit.Enabled = false;
                return;
            }

            int deptID;
            if (!int.TryParse(deptIdStr, out deptID))
            {
                ShowAlert("Invalid Department ID specified.", "alert-error");
                btnSubmit.Enabled = false;
                return;
            }

            int? subDeptID = null;
            string subDeptIdStr = Request.QueryString["SubDeptId"];
            int parsedSubDeptID;
            if (!string.IsNullOrEmpty(subDeptIdStr) && int.TryParse(subDeptIdStr, out parsedSubDeptID))
            {
                subDeptID = parsedSubDeptID;
            }

            int? locationID = null;
            string locationIdStr = Request.QueryString["LocationId"];
            int parsedLocationID;
            if (!string.IsNullOrEmpty(locationIdStr) && int.TryParse(locationIdStr, out parsedLocationID))
            {
            ...
            try
            {
                // Retrieve Department and Subdepartment names dynamically using helper
                DataTable dtDept = DBHelper.GetBasicDataTableData("SELECT Dept_Name FROM Department WHERE Dept_ID = " + deptID);
                if (dtDept.Rows.Count == 0)
                {
                    ShowAlert("Department not found.", "alert-error");
                    btnSubmit.Enabled = false;
                    return;
                }

                string deptName = dtDept.Rows[0]["Dept_Name"].ToString();
                string subDeptName = "";

                if (subDeptID.HasValue)
                {
                    DataTable dtSubDept = DBHelper.GetBasicDataTableData("SELECT SubDept_Name FROM SubDepartment WHERE SubDept_Id = " + subDeptID.Value);
                    if (dtSubDept.Rows.Count > 0)
                    {
                        subDeptName = dtSubDept.Rows[0]["SubDept_Name"].ToString();
                    }
                }
                ...
                {
                    DataTable dtLoc = DBHelper.GetTableData("SELECT LocationName FROM dbo.FeedbackLocations WHERE LocationID = " + locationID.Value);
                    if (dtLoc.Rows.Count > 0)
                    {
                        locationName = dtLoc.Rows[0]["LocationName"].ToString();
                    }
                }

                ViewState["DeptID"] = deptID;
                ViewState["SubDeptID"] = subDeptID;
                ViewState["LocationID"] = locationID;
                ViewState["FormToken"] = Guid.NewGuid().ToString();

                string displayName = deptName;
                if (!string.IsNullOrEmpty(subDeptName))
                {
                    displayName += " - " + subDeptName;
                }
                ...
                litDeptName.Text = displayName;
                litFeedbackTitle.Text = "Feedback & Service Review";
                litInstructions.Text = "<div style='font-size: 14px; color: #64748b; line-height: 1.6; margin-bottom: 24px; background: #f8fafc; padding: 12px; border-radius: 8px; border-left: 3px solid #c5a059; text-align: left;'>Please rate your experience across the criteria below. Your rating helps us maintain premium club services.</div>";

                // Bind dynamic feedback questions
                BindQuestions(deptID, subDeptID, locationID);
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading department details: " + ex.Message, "alert-error");
                btnSubmit.Enabled = false;
            }
        }

        private void BindQuestions(int deptID, int? subDeptID, int? locationID)
        {
            try
            {
                DataTable dt = DBHelper.GetFeedbackQuestions(deptID, subDeptID, locationID, true);
                if (dt.Rows.Count > 0)
                {
                    repQuestions.DataSource = dt;
                    repQuestions.DataBind();
                    ViewState["QuestionsCount"] = dt.Rows.Count;
                }
                else
                {
                    // Fallback: If no location questions are defined, try to load subdepartment-level questions
                    if (locationID.HasValue)
                    {
                        DataTable dtSubOnly = DBHelper.GetFeedbackQuestions(deptID, subDeptID, null, true);
                        if (dtSubOnly.Rows.Count > 0)
                        {
                            repQuestions.DataSource = dtSubOnly;
                            repQuestions.DataBind();
                            ViewState["QuestionsCount"] = dtSubOnly.Rows.Count;
                            return;
                        }
                    }

                    // Fallback: If no subdepartment questions are defined, try to load department-level questions
                    if (subDeptID.HasValue)
                    {
                        DataTable dtDeptOnly = DBHelper.GetFeedbackQuestions(deptID, null, null, true);
                        if (dtDeptOnly.Rows.Count > 0)
                        {
                            repQuestions.DataSource = dtDeptOnly;
                            repQuestions.DataBind();
                            ViewState["QuestionsCount"] = dtDeptOnly.Rows.Count;
                            return;
                        }
                    }

                    ShowAlert("No active feedback questions are currently set up for this location.", "alert-error");
                    btnSubmit.Enabled = false;
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading feedback questions: " + ex.Message, "alert-error");
                btnSubmit.Enabled = false;
            }
        }
        ...
        protected void repQuestions_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string qType = drv["QuestionType"] != DBNull.Value ? drv["QuestionType"].ToString() : "Rating";
                if (qType != "Rating")
                {
                    Repeater repOptionItems = (Repeater)e.Item.FindControl("repOptionItems");
                    if (repOptionItems != null)
                    {
                        ...
                        {
                            string optsStr = drv["Options"] != DBNull.Value ? drv["Options"].ToString() : "";
                            if (!string.IsNullOrEmpty(optsStr))
                            {
                                ...
                                foreach (string p in parts)
                                {
                                    if (!string.IsNullOrWhiteSpace(p))
                                        optionsList.Add(p.Trim());
                                }
                            }
                        }
                        repOptionItems.DataSource = optionsList;
                        repOptionItems.DataBind();
                    }
                }
            }
        }

        protected void chkIsComplaint_CheckedChanged(object sender, EventArgs e)
        {
            if (chkIsComplaint.Checked)
            {
                pnlRating.Visible = false;
                pnlSubject.Visible = true;
                btnSubmit.Text = "Submit Complaint";
            ...
            else
            {
                ...
                litCommentsLabel.Text = "General Comments & Suggestions";
            }
        }

        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (ViewState["DeptID"] == null) return;

            string formToken = ViewState["FormToken"] as string;
            if (!string.IsNullOrEmpty(formToken))
            {
                if (Session["SubmittedToken_" + formToken] != null)
                {
                    pnlForm.Visible = false;
                    pnlSuccess.Visible = true;
                    return;
                }
                if (Session["IsSubmitting_" + formToken] != null && (bool)Session["IsSubmitting_" + formToken])
                {
                    return;
                }
            }

            // Hard-enforce complaint mode regardless of what was submitted client-side
            chkIsComplaint.Checked = true;

            int deptID = (int)ViewState["DeptID"];
            int? subDeptID = (int?)ViewState["SubDeptID"];
            int? locationID = (int?)ViewState["LocationID"];

            string memberNo = txtMemberNo.Text.Trim();
            if (string.IsNullOrEmpty(memberNo)) memberNo = null;

            string memberName = txtMemberName.Text.Trim();
            string email = txtEmail.Text.Trim();
            string phone = txtPhone.Text.Trim();

            // Enforce that at least one information field is provided
            if (string.IsNullOrEmpty(memberNo) && string.IsNullOrEmpty(memberName) && string.IsNullOrEmpty(email) && string.IsNullOrEmpty(phone))
            {
                ShowAlert("Please fill in at least one information detail: Membership Number, Member Name, Email, or Phone Number.", "alert-error");
                return;
            }
            ...
            if (chkIsComplaint.Checked)
            {
                if (string.IsNullOrEmpty(subject) && string.IsNullOrEmpty(comments))
                {
                    ShowAlert("Please enter a complaint subject or detail description before submitting.", "alert-error");
                    return;
                }
                if (string.IsNullOrEmpty(subject)) subject = "General Complaint";
            }

            // Prevent duplicate submissions with identical details within 60 seconds
            string payloadSig = string.Format("{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}", deptID, subDeptID, locationID, memberNo, memberName, email, subject, comments);
            if (Session["LastSubSig"] != null && Session["LastSubTime"] != null)
            {
                string lastSig = (Session["LastSubSig"] != null ? Session["LastSubSig"].ToString() : "");
                DateTime lastTime = (DateTime)Session["LastSubTime"];
                if (lastSig == payloadSig && (DateTime.Now - lastTime).TotalSeconds < 60)
                {
                    pnlForm.Visible = false;
                    pnlSuccess.Visible = true;
                    return;
            ...
            if (!string.IsNullOrEmpty(formToken))
            {
                Session["IsSubmitting_" + formToken] = true;
            }

            // Append contact details to comments / detail text if provided
            StringBuilder detailBuilder = new StringBuilder();
            if (!string.IsNullOrEmpty(comments))
                detailBuilder.AppendLine(comments);

            if (!string.IsNullOrEmpty(memberName) || !string.IsNullOrEmpty(email) || !string.IsNullOrEmpty(phone))
            {
                if (detailBuilder.Length > 0) detailBuilder.AppendLine();
                detailBuilder.AppendLine("--- Contact Details ---");
                if (!string.IsNullOrEmpty(memberName)) detailBuilder.AppendLine("Member Name: " + memberName);
                if (!string.IsNullOrEmpty(email)) detailBuilder.AppendLine("Email: " + email);
                if (!string.IsNullOrEmpty(phone)) detailBuilder.AppendLine("Phone: " + phone);
            }
            string finalComments = detailBuilder.ToString();
            ...
            try
            {
                ...
                {
                    // Submit Complaint
                    string complaintTrackingId;
                    string result = DBHelper.SubmitComplaint(deptID, subDeptID, locationID, memberNo, memberName, email, phone, subject, string.IsNullOrEmpty(comments) ? subject : comments, out complaintTrackingId);
                    if (result.Contains("successfully"))
                    {
                        litSuccessMessage.Text = "Your complaint has been successfully registered. Our administration will inspect it and take necessary action.";
                        pnlForm.Visible = false;
                        pnlSuccess.Visible = true;

                        Session["LastSubSig"] = payloadSig;
                        Session["LastSubTime"] = DateTime.Now;
                        if (!string.IsNullOrEmpty(formToken))
                        {
                            Session["SubmittedToken_" + formToken] = true;
                        }

                        // ── Send confirmation email to member (if email provided) ──
                        if (!string.IsNullOrEmpty(email))
                        {
                            string emailErr = EmailHelper.SendFeedbackConfirmationEmail(email, memberNo, memberName, complaintTrackingId);
                            if (!string.IsNullOrEmpty(emailErr))
                                litSuccessMessage.Text += "<br/><small style='color:#94a3b8;font-size:11px;'>Email notice: " + System.Web.HttpUtility.HtmlEncode(emailErr) + "</small>";
                        }
                    }
                    else
                    {
                ...
                {
                    // Submit Multiple Question Feedback
                    string[] questionIDs = Request.Form.GetValues("question_id");

                    StringBuilder xmlBuilder = new StringBuilder();
                    xmlBuilder.Append("<ratings>");
                    ...
                    if (questionIDs != null && questionIDs.Length > 0)
                    {
                        ...
                        {
                            string qType = Request.Form["qtype_" + qID];
                            if (string.IsNullOrEmpty(qType)) qType = "Rating";

                            if (qType == "Rating")
                            {
                                string ratingVal = Request.Form["rating_" + qID];
                                if (!string.IsNullOrEmpty(ratingVal))
                                {
                                    hasAnyRating = true;
                                    xmlBuilder.AppendFormat("<rating questionId=\"{0}\" value=\"{1}\" answerText=\"{2}\"/>",
                                        qID, ratingVal, System.Web.HttpUtility.HtmlEncode(ratingVal + " Stars"));
                                }
                            }
                            else
                            {
                                string answerVal = Request.Form["answer_" + qID];
                                if (!string.IsNullOrEmpty(answerVal))
                                {
                                    hasAnyRating = true;
                                    xmlBuilder.AppendFormat("<rating questionId=\"{0}\" value=\"\" answerText=\"{1}\"/>",
                                        qID, System.Web.HttpUtility.HtmlEncode(answerVal));
                                }
                            }
                        }
                    }
                    ...
                    // Enforce that at least one rating or comment is provided overall
                    if (!hasAnyRating && string.IsNullOrEmpty(comments))
                    {
                        ShowAlert("Please provide at least one rating or comment before submitting.", "alert-error");
                        return;
                    }

                    string feedbackTrackingId;
                    string result = DBHelper.SubmitFeedbackMultiple(deptID, subDeptID, locationID, memberNo, memberName, email, phone, comments, xmlBuilder.ToString(), out feedbackTrackingId);
                    if (result.Contains("successfully"))
                    {
                        litSuccessMessage.Text = "Your feedback has been successfully submitted. Thank you for helping us maintain premium standards at Lahore Gymkhana.";
                        pnlForm.Visible = false;
                        pnlSuccess.Visible = true;
                        ...
                        if (!string.IsNullOrEmpty(email))
                        {
                            string emailErr = EmailHelper.SendFeedbackConfirmationEmail(email, memberNo, memberName, feedbackTrackingId);
                            if (!string.IsNullOrEmpty(emailErr))
                                litSuccessMessage.Text += "<br/><small style='color:#94a3b8;font-size:11px;'>Email notice: " + System.Web.HttpUtility.HtmlEncode(emailErr) + "</small>";
                        }
                    }
            ...
            {
                ShowAlert("Error submitting details: " + ex.Message, "alert-error");
            }
            finally
            {
                if (!string.IsNullOrEmpty(formToken))
                {
                    Session.Remove("IsSubmitting_" + formToken);
                }
            }
        }
        ...
        #region Nested Helper Classes (DBHelper & EmailHelper & ISBN13Helper)

        /// <summary>
        /// Centralised database access helper for Lahore Gymkhana Library.
        /// All communication uses Stored Procedures — no inline SQL.
        ...
        public static class DBHelper
        {
            ...
            // ─────────────────────────────────────────────────────────────────
            //  Execute SP → DataTable (SELECT results)
            // ─────────────────────────────────────────────────────────────────
            ...
            // Execute SP → DataSet (multiple result sets, e.g. sp_GetBookDetail)
            ...
            // ─────────────────────────────────────────────────────────────────
            //  Execute SP → no return value (fire-and-forget DML)
            // ─────────────────────────────────────────────────────────────────
            ...
            /// <summary>
            /// Submits multiple-question feedback. Returns the SP result message and, via the
            /// out parameter, the newly created ComplaintID (used as the tracking ID) —
            /// requires sp_SubmitFeedbackMultiple to declare @ComplaintID OUTPUT internally
            /// (e.g. SET @ComplaintID = SCOPE_IDENTITY(); after the INSERT into Complaint).
            /// If the SP does not yet return this value, complaintId will be null.
            /// </summary>
            public static string SubmitFeedbackMultiple(int deptID, int? subDeptID, int? locationID, string memberNo, string memberName, string email, string phoneNumber, string comments, string ratingsXml, out string complaintId)
            {
                complaintId = null;
                try
                {
                    var prms = new[]
                    {
                        ...
                        new SqlParameter("@MemberName",      (object)memberName ?? DBNull.Value),
                    ...
                    int newId = GetOutputValue<int>(prms, "@ComplaintID");
                    complaintId = newId > 0 ? newId.ToString() : null;
                    return GetOutputValue<string>(prms, "@Msg");
                }
                catch (Exception ex)
                {
                    if (ex.Message != null && ex.Message.Contains("too many arguments"))
                    {
                        StringBuilder sbComments = new StringBuilder();
                        if (!string.IsNullOrEmpty(comments)) sbComments.AppendLine(comments);
                        if (!string.IsNullOrEmpty(memberName) || !string.IsNullOrEmpty(email) || !string.IsNullOrEmpty(phoneNumber))
                        {
                            sbComments.AppendLine("\n--- Contact Info ---");
                            if (!string.IsNullOrEmpty(memberName)) sbComments.AppendLine("Name: " + memberName);
                            if (!string.IsNullOrEmpty(email)) sbComments.AppendLine("Email: " + email);
                            if (!string.IsNullOrEmpty(phoneNumber)) sbComments.AppendLine("Phone: " + phoneNumber);
                        }

                        var fallbackPrms = new[]
                        {
                            new SqlParameter("@DeptID",          deptID),
                            new SqlParameter("@SubDeptID",       (object)subDeptID ?? DBNull.Value),
                            new SqlParameter("@MemberNo",        (object)memberNo ?? DBNull.Value),
                            new SqlParameter("@GeneralComments", sbComments.ToString()),
                            new SqlParameter("@RatingsXml",      ratingsXml),
                            new SqlParameter("@ComplaintID",     SqlDbType.Int) { Direction = ParameterDirection.Output },
                            new SqlParameter("@Msg",             SqlDbType.VarChar, 200) { Direction = ParameterDirection.Output }
                        };
                        ExecuteNonQuery("dbo.sp_SubmitFeedbackMultiple", fallbackPrms);
                        int fallbackId = GetOutputValue<int>(fallbackPrms, "@ComplaintID");
                        complaintId = fallbackId > 0 ? fallbackId.ToString() : null;
                        return GetOutputValue<string>(fallbackPrms, "@Msg");
                    }
                    throw;
                }
            }
            ...
            /// <summary>
            /// Submits a complaint. Returns the SP result message and, via the out parameter,
            /// the newly created ComplaintID (used as the tracking ID) — requires
            /// sp_SubmitComplaint to declare @ComplaintID OUTPUT internally
            /// (e.g. SET @ComplaintID = SCOPE_IDENTITY(); after the INSERT into Complaint).
            /// If the SP does not yet return this value, complaintId will be null.
            /// </summary>
            public static string SubmitComplaint(int? deptID, int? subDeptID, int? locationID, string memberNo, string memberName, string email, string phoneNumber, string subject, string detail, out string complaintId)
            {
                complaintId = null;
                // Tier 1: Try with full parameter set including @ComplaintID OUTPUT
                try
                {
                    var prms = new[]
                    {
                ...
                {
                    // Tier 2: Try with Contact & Location parameters but without @ComplaintID OUTPUT
                    try
                    {
                        var prms2 = new[]
                        {
                        ...
                        ExecuteNonQuery("dbo.sp_SubmitComplaint", prms2);
                        return GetOutputValue<string>(prms2, "@Msg");
                    }
                    catch (Exception)
                    {
                        // Tier 3: Try standard parameters without Location/Contact columns (appending contact info to detail)
                        StringBuilder sbDetail = new StringBuilder();
                        if (!string.IsNullOrEmpty(detail)) sbDetail.AppendLine(detail);
                        if (!string.IsNullOrEmpty(memberName) || !string.IsNullOrEmpty(email) || !string.IsNullOrEmpty(phoneNumber))
                        {
                            sbDetail.AppendLine("\n--- Contact Info ---");
                            if (!string.IsNullOrEmpty(memberName)) sbDetail.AppendLine("Name: " + memberName);
                            if (!string.IsNullOrEmpty(email)) sbDetail.AppendLine("Email: " + email);
                            if (!string.IsNullOrEmpty(phoneNumber)) sbDetail.AppendLine("Phone: " + phoneNumber);
                        }
                        ...
                        var fallbackPrms = new[]
                        {
                            ...
                            new SqlParameter("@MemberNo",         (object)memberNo ?? DBNull.Value),
                            new SqlParameter("@ComplaintSubject",  subject),
                            new SqlParameter("@ComplaintDetail",   sbDetail.ToString()),
                            new SqlParameter("@Msg",              SqlDbType.VarChar, 200) { Direction = ParameterDirection.Output }
                        };
                        ExecuteNonQuery("dbo.sp_SubmitComplaint", fallbackPrms);
                        return GetOutputValue<string>(fallbackPrms, "@Msg");
                    }
                }
            }
            ...
            {
                return GetFeedbackQuestions(deptID, subDeptID, null, activeOnly);
            }

            public static DataTable GetFeedbackQuestions(int deptID, int? subDeptID, int? locationID, bool activeOnly)
            {
                var prms = new[]
                {
                    new SqlParameter("@DeptID", deptID),
        ...
        /// <summary>
        /// Email utility for Lahore Gymkhana Library - FIXED VERSION
        /// Uses complaint@lahoregymkhana.org.pk via SMTP.
        /// Optimized for precompiled applications - no external dependencies.
        /// </summary>
        public static class EmailHelper
        {
            // ── SMTP credentials ────────────────────────────────────────────────────
            private const string SmtpHost = "mail.lahoregymkhana.org.pk";
            private const string SmtpUser = "complaint@lahoregymkhana.org.pk";
            private const string SmtpPassword = "LhrGymkhana";
            private const string FromName = "Lahore Gymkhana";

            // ── Core send method ────────────────────────────────────────────────────
            /// <summary>
            /// Sends an HTML email. Returns empty string on success, error message on failure.
            /// </summary>
            public static string SendEmail(string toAddress, string toName, string subject, string htmlBody)
            {
                if (string.IsNullOrWhiteSpace(toAddress))
                    return "No recipient address.";

                // Bypass certificate validation for self-signed certs
                System.Net.ServicePointManager.ServerCertificateValidationCallback =
                    (sender, certificate, chain, sslPolicyErrors) => true;

                // Force TLS 1.0 | 1.1 | 1.2
                System.Net.ServicePointManager.SecurityProtocol =
                    System.Net.SecurityProtocolType.Tls
                    | System.Net.SecurityProtocolType.Tls11
                    | System.Net.SecurityProtocolType.Tls12;

                // Try port 587 (STARTTLS) first, then port 25, then port 465
                var attempts = new[]
                {
                    new { Port = 587, Ssl = true,  Label = "587 STARTTLS" },
                    new { Port = 25,  Ssl = false, Label = "25 plain" },
                    new { Port = 465, Ssl = true,  Label = "465 SSL" },
                };
                ...
                foreach (var attempt in attempts)
                {
                    try
                    {
                        ...
                        {
                            msg.From = new MailAddress(SmtpUser, FromName);
                            msg.To.Add(new MailAddress(toAddress.Trim(),
                                           string.IsNullOrEmpty(toName) ? toAddress : toName));
                            msg.Subject = subject;
                            msg.Body = htmlBody;
                            msg.IsBodyHtml = true;
                            ...
                            using (var smtp = new SmtpClient(SmtpHost, attempt.Port))
                            {
                                smtp.EnableSsl = attempt.Ssl;
                                smtp.Credentials = new System.Net.NetworkCredential(SmtpUser, SmtpPassword);
                                smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
                                smtp.Timeout = 15000;

                                // Try to set HELO domain via reflection (fixes "Invalid HELO name" error)
                                SetHeloDomain(smtp, "lahoregymkhana.org.pk");
                    ...
                    {
                        lastError = string.Format("Port {0} ssl={1}: {2}{3}",
                            attempt.Port, attempt.Ssl, ex.Message,
                            ex.InnerException != null ? " | Inner: " + ex.InnerException.Message : "");
                    }
                }

                return "Email delivery failed (all ports tried). Last error: " + lastError;
            }

            // ── HELO Domain Fix via Reflection ──────────────────────────────────────
            /// <summary>
            /// Sets the HELO/EHLO domain name on the SmtpClient via reflection.
            /// Fixes "Invalid HELO name" (RFC2821 4.1.1.1) errors.
            /// </summary>
            private static void SetHeloDomain(SmtpClient smtp, string domain)
            {
                try
                {
                    // Try different field names that might exist in different .NET versions
                    string[] fieldNames = { 
                        "localHostName", 
                        "_localHostName", 
                        "m_localHostName", 
                    ...
                    {
                        var field = typeof(SmtpClient).GetField(fieldName,
                    ...
                    // If field not found, try property
                    var prop = typeof(SmtpClient).GetProperty("LocalHostName",
                        System.Reflection.BindingFlags.Instance |
                        System.Reflection.BindingFlags.NonPublic |
                        System.Reflection.BindingFlags.Public |
                        System.Reflection.BindingFlags.IgnoreCase);
                    ...
                    {
                        prop.SetValue(smtp, domain, null);
                    }
                }
                catch
                {
                    // Ignore reflection errors - the email might still work
                }
            }

            // ── Feedback confirmation ────────────────────────────────────────────────
            /// <summary>
            /// Sends the feedback confirmation email. trackingId is the ComplaintID from the
            /// Complaint table (used as the tracking reference) — pass null if not available,
            /// in which case the email will display "N/A".
            /// </summary>
            public static string SendFeedbackConfirmationEmail(string toEmail, string memberNo, string memberName, string trackingId)
            {
                string subject = "Thank You for Your Feedback – Lahore Gymkhana";
                string body = GetFeedbackEmailBody(memberNo, memberName, toEmail, trackingId);
                return SendEmail(toEmail, memberName, subject, body);
            }

            // ── Complaint confirmation ───────────────────────────────────────────────
            public static string SendComplaintConfirmationEmail(string toEmail, string memberName, string complaintSubject, string departmentName)
            {
                string displayName = string.IsNullOrEmpty(memberName) ? "Valued Member" : memberName;
                string subject = "Complaint Received – Lahore Gymkhana";
                string body = GetComplaintEmailBody(displayName, departmentName, complaintSubject);
                return SendEmail(toEmail, memberName, subject, body);
            }

            // ── HTML Email Templates ──────────────────────────────────────────────────
            private static string GetFeedbackEmailBody(string memberNo, string memberName, string email, string trackingId)
            {
                string displayName = string.IsNullOrEmpty(memberName) ? "Valued Member" : memberName;
                string memberNoDisplay = string.IsNullOrEmpty(memberNo) ? "" : memberNo;
                string trackingDisplay = string.IsNullOrEmpty(trackingId) ? "N/A" : trackingId;
                string emailDisplay = string.IsNullOrEmpty(email) ? "" : email;
  ...
  <table width='100%' cellpadding='0' cellspacing='0' style='background:#f8fafc;padding:32px 0;'>
    <tr><td align='center'>
      <table width='520' cellpadding='0' cellspacing='0' style='background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;'>
        <tr>
          <td style='background:linear-gradient(135deg,#0f1e36,#1c3254);padding:28px 32px;'>
            <table width='100%' cellpadding='0' cellspacing='0'>
              <tr>
                <td align='left' valign='top'>
                  <div style='width:52px;height:52px;background:linear-gradient(135deg,#c5a059,#aa8441);border-radius:50%;display:inline-block;line-height:52px;font-family:Georgia,serif;font-size:28px;font-weight:700;color:#0f1e36;text-align:center;'>G</div>
                  <div style='margin-top:10px;font-family:Georgia,serif;font-size:20px;font-weight:700;color:#c5a059;letter-spacing:1px;'>LAHORE GYMKHANA</div>
                  <div style='font-size:11px;color:#94a3b8;letter-spacing:2px;text-transform:uppercase;margin-top:4px;'>Member Feedback</div>
                </td>
                <td align='right' valign='top'>
                  <div style='font-size:10px;color:#94a3b8;letter-spacing:1px;text-transform:uppercase;'>Tracking ID</div>
                  <div style='font-size:14px;color:#c5a059;font-weight:700;margin-top:2px;'>{3}</div>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td style='padding:32px;'>
            <p style='font-size:16px;color:#0f1e36;font-weight:600;margin:0 0 16px 0;'>Dear {0} {1},</p>
            ...
            <p style='font-size:14px;color:#475569;line-height:1.7;margin:0 0 24px 0;'>
              Your input has been recorded and will be reviewed by our team. Member feedback is essential
        ...
        <tr>
          <td style='background:#f8fafc;padding:16px 32px;text-align:center;border-top:1px solid #e2e8f0;'>
            <p style='font-size:11px;color:#94a3b8;margin:0 0 6px 0;'>{2}</p>
            <p style='font-size:11px;color:#94a3b8;margin:0;'>
              &copy; Lahore Gymkhana &mdash; 
              <a href='https://lahoregymkhana.org.pk' style='color:#c5a059;text-decoration:none;'>lahoregymkhana.org.pk</a>
            </p>
          </td>
        </tr>
      </table>
...
</html>", memberNoDisplay, displayName, emailDisplay, trackingDisplay);
            }
            private static string GetComplaintEmailBody(string displayName, string departmentName, string complaintSubject)
            {
                return string.Format(@"
        <html>
        <body style='margin:0;padding:0;background:#f8fafc;font-family:Calibri,Arial,sans-serif;'>
          <table width='100%' cellpadding='0' cellspacing='0' style='background:#f8fafc;padding:32px 0;'>
            <tr><td align='center'>
              <table width='520' cellpadding='0' cellspacing='0' style='background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;'>
                <tr>
                  <td style='background:linear-gradient(135deg,#0f1e36,#1c3254);padding:28px 32px;text-align:center;'>
                    <div style='width:52px;height:52px;background:linear-gradient(135deg,#c5a059,#aa8441);border-radius:50%;display:inline-block;line-height:52px;font-family:Georgia,serif;font-size:28px;font-weight:700;color:#0f1e36;'>G</div>
                    <div style='margin-top:10px;font-family:Georgia,serif;font-size:20px;font-weight:700;color:#c5a059;letter-spacing:1px;'>LAHORE GYMKHANA</div>
                    <div style='font-size:11px;color:#94a3b8;letter-spacing:2px;text-transform:uppercase;margin-top:4px;'>Complaint Acknowledgement</div>
                  </td>
                </tr>
                <tr>
                  <td style='padding:32px;'>
                    <p style='font-size:16px;color:#0f1e36;font-weight:600;margin:0 0 16px 0;'>Dear {0},</p>
                    <p style='font-size:14px;color:#475569;line-height:1.7;margin:0 0 16px 0;'>
                      We have received your complaint regarding <strong style='color:#0f1e36;'>{1}</strong>
                      and it has been formally registered with our administration.
                    </p>
                    <table width='100%' cellpadding='0' cellspacing='0' style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:20px;'>
                      <tr>
                        <td style='padding:6px 16px;font-size:12px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid #e2e8f0;'>Complaint Subject</td>
                      </tr>
                      <tr>
                        <td style='padding:12px 16px;font-size:14px;color:#0f1e36;font-weight:600;'>{2}</td>
                      </tr>
                    </table>
                    <p style='font-size:14px;color:#475569;line-height:1.7;margin:0 0 24px 0;'>
                      Our administration team will review your complaint and take the necessary action.
                      We aim to respond within <strong style='color:#0f1e36;'>3–5 working days</strong>.
                    </p>
                    <div style='border-top:1px solid #e2e8f0;margin:24px 0;'></div>
                    <p style='font-size:13px;color:#64748b;line-height:1.6;margin:0;'>
                      For follow-ups, please contact our administration office.
                    </p>
                  </td>
                </tr>
                <tr>
        ...
        </html>", displayName, departmentName, complaintSubject);
            }
        }

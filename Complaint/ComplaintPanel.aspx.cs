...
{
    public partial class Pages_ComplaintPanel : System.Web.UI.Page
    {
        ...
        {
            object val = Session["Emp_ID"] ?? Session["EmpID"] ?? Session["StaffID"];
            if (val != null)
            {
                decimal tempEmpId;
                if (decimal.TryParse(val.ToString(), out tempEmpId) && tempEmpId > 0)
                {
                    return tempEmpId;
                }
            }
        ...
        private void BindDepartments()
        {
            try
            {
                ...
                DataTable dt = DBHelper.GetAuthorizedTargetDepartments(viewingEmpId);
        ...
        protected void ddlDeptFilter_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlDeptFilter.SelectedValue != "0" && !string.IsNullOrEmpty(ddlDeptFilter.SelectedValue))
            {
                BindSubDepartments(Convert.ToInt32(ddlDeptFilter.SelectedValue));
            }
            else
            {
                ddlSubDeptFilter.Items.Clear();
                ddlSubDeptFilter.Enabled = false;
            }
            BindComplaints();
        ...
        private void BindSubDepartments(int deptId)
        {
            ...
            {
                decimal? viewingEmpId = GetViewingEmpID();
                DataTable dt = DBHelper.GetAuthorizedSubDepartments(deptId, viewingEmpId);
                ddlSubDeptFilter.Items.Clear();
                if (dt != null && dt.Rows.Count > 0)
                {
                    ddlSubDeptFilter.DataSource = dt;
                    ddlSubDeptFilter.DataTextField = "SubDept_Name";
                    ddlSubDeptFilter.DataValueField = "SubDept_Id";
                    ddlSubDeptFilter.DataBind();
                    ddlSubDeptFilter.Items.Insert(0, new ListItem("- All Subdepartments -", "0"));
                    ddlSubDeptFilter.Enabled = true;
                }
                else
                {
                    ddlSubDeptFilter.Items.Insert(0, new ListItem("- No Subdepartments -", "0"));
                    ddlSubDeptFilter.Enabled = false;
                }
            }
            catch (Exception ex)
        ...
        private void BindComplaints()
        {
            ...
            {
                decimal? viewingEmpId = GetViewingEmpID();
                int? deptId = (ddlDeptFilter.SelectedValue != "0" && !string.IsNullOrEmpty(ddlDeptFilter.SelectedValue)) ? (int?)Convert.ToInt32(ddlDeptFilter.SelectedValue) : null;
                int? subDeptId = null;
                if (ddlSubDeptFilter.Enabled && ddlSubDeptFilter.SelectedValue != "0" && !string.IsNullOrEmpty(ddlSubDeptFilter.SelectedValue))
                {
                    subDeptId = Convert.ToInt32(ddlSubDeptFilter.SelectedValue);
                }
                ...
                DataTable dt = DBHelper.GetComplaints(deptId, subDeptId, type, status, fromDate, toDate, viewingEmpId);
                if (dt != null)
                {
                    if (!dt.Columns.Contains("LocationName"))
                    {
                ...
                if (viewingEmpId.HasValue && (dt == null || dt.Rows.Count == 0))
                {
                    bool hasAuth = DBHelper.HasEmployeeComplaintAuthority(viewingEmpId.Value);
                    if (!hasAuth)
                    {
                        ShowAlert("Access Notice: No departmental complaint authority has been allocated to your employee account. Please contact an administrator to assign department/subdepartment permissions in the Complaint Authority setup.", "alert-error");
                    }
                }
            }
            catch (Exception ex)
            {
                ShowAlert("Error loading submissions: " + ex.Message, "alert-error");
            }
        }
        ...
        protected void gvComplaints_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "ManageComplaint")
            {
                try
                {
                    ...
                    DataTable dt = DBHelper.GetComplaints(null, null, recordType, null, null, null, viewingEmpId);
                    DataRow[] rows = dt.Select("ID = " + id + " AND RecordType = '" + recordType.Replace("'", "''") + "'");
                    if (rows.Length > 0)
                    {
                        DataRow r = rows[0];
                        hfComplaintID.Value = r["ID"].ToString();
                        hfRecordType.Value = r["RecordType"].ToString();
                        ViewState["SelectedType"] = r["RecordType"].ToString();
                        
                        litCompSubject.Text = r["Subject"].ToString();
                        
                        string memberName = r["MemberName"] != DBNull.Value ? r["MemberName"].ToString().Trim() : "Guest / Anonymous Member";
                        string memberNo = (dt.Columns.Contains("MemberNo") && r["MemberNo"] != DBNull.Value) ? r["MemberNo"].ToString().Trim() : "";
                        string guestEmail = (dt.Columns.Contains("Email") && r["Email"] != DBNull.Value) ? r["Email"].ToString().Trim() : "";
                        string guestPhone = (dt.Columns.Contains("PhoneNumber") && r["PhoneNumber"] != DBNull.Value) ? r["PhoneNumber"].ToString().Trim() : "";
                        
                        string deptName = (dt.Columns.Contains("DepartmentName") && r["DepartmentName"] != DBNull.Value) ? r["DepartmentName"].ToString().Trim() : "";
                        string subDeptName = (dt.Columns.Contains("SubDepartmentName") && r["SubDepartmentName"] != DBNull.Value) ? r["SubDepartmentName"].ToString().Trim() : "";
                        string locationName = (dt.Columns.Contains("LocationName") && r["LocationName"] != DBNull.Value) ? r["LocationName"].ToString().Trim() : "";
                        string createdDateStr = (dt.Columns.Contains("CreatedDate") && r["CreatedDate"] != DBNull.Value) ? Convert.ToDateTime(r["CreatedDate"]).ToString("dd-MMM-yyyy hh:mm tt") : DateTime.Now.ToString("dd-MMM-yyyy");

                        hfMemberNo.Value = memberNo;
                        hfMemberName.Value = memberName;
                        hfMemberEmail.Value = guestEmail;
                        hfMemberPhone.Value = guestPhone;
                        hfDepartmentName.Value = deptName;
                        hfSubDepartmentName.Value = subDeptName;
                        hfLocationName.Value = locationName;
                        hfCreatedDate.Value = createdDateStr;

                        string displayMemberNo = !string.IsNullOrEmpty(memberNo) ? " (M.No: " + memberNo + ")" : "";
                        string displayEmail = !string.IsNullOrEmpty(guestEmail) ? " &middot; Email: " + guestEmail : "";
                        string displayPhone = !string.IsNullOrEmpty(guestPhone) ? " &middot; Phone: " + guestPhone : "";
                        
                        litCompMember.Text = memberName + displayMemberNo + displayEmail + displayPhone;
                        litCompLocation.Text = string.IsNullOrEmpty(locationName) ? (string.IsNullOrEmpty(subDeptName) ? (string.IsNullOrEmpty(deptName) ? "N/A" : deptName) : deptName + " - " + subDeptName) : locationName;
                        litCompDetail.Text = r["Detail"].ToString();
                        
                        // Populate status drop-down options based on type
                        ddlUpdateStatus.Items.Clear();
                        if (recordType == "Feedback")
                        {
                            ddlUpdateStatus.Items.Add(new ListItem("Submitted", "Submitted"));
                            ddlUpdateStatus.Items.Add(new ListItem("Reviewed & Logged", "Reviewed"));
                        }
                        else
                        {
                            ddlUpdateStatus.Items.Add(new ListItem("Pending", "Pending"));
                            ddlUpdateStatus.Items.Add(new ListItem("In Progress", "In Progress"));
                ...
                {
                    ShowAlert("Error loading details: " + ex.Message, "alert-error");
                }
            }
        }
        ...
        {
            if (string.IsNullOrEmpty(hfComplaintID.Value) || ViewState["SelectedType"] == null) return;

            int id = Convert.ToInt32(hfComplaintID.Value);
            string type = ViewState["SelectedType"].ToString();
            ...
            try
            {
                string msg = DBHelper.UpdateComplaintStatus(id, type, status, remarks);
        ...
        public string FormatMemberDetails(object nameObj, object noObj, object emailObj, object phoneObj)
        {
            string name = nameObj != null && nameObj != DBNull.Value ? nameObj.ToString().Trim() : "";
            string no = noObj != null && noObj != DBNull.Value ? noObj.ToString().Trim() : "";
            string email = emailObj != null && emailObj != DBNull.Value ? emailObj.ToString().Trim() : "";
            string phone = phoneObj != null && phoneObj != DBNull.Value ? phoneObj.ToString().Trim() : "";
            ...
            if (!string.IsNullOrEmpty(no)) subInfo.Add("M.No: " + no);
            if (!string.IsNullOrEmpty(email)) subInfo.Add("Email: " + email);
            if (!string.IsNullOrEmpty(phone)) subInfo.Add("Phone: " + phone);

            string html = "<div style='font-weight: 600; color: #0f1e36;'>" + Context.Server.HtmlEncode(name) + "</div>";
            if (subInfo.Count > 0)
            {
                html += "<div style='font-size: 12px; color: #64748b; margin-top: 2px;'>" + Context.Server.HtmlEncode(string.Join(" · ", subInfo)) + "</div>";
            }
            return html;
        }

        public string GetLocationDisplay(object locationNameObj)
        {
            if (locationNameObj == null || locationNameObj == DBNull.Value) return "-";
            string loc = locationNameObj.ToString().Trim();
            return string.IsNullOrEmpty(loc) ? "-" : Context.Server.HtmlEncode(loc);
        }
    ...
    {
        public static DataTable GetEmployeeDepartment(int empId)
        {
            try
            {
                string query = @"
                    SELECT ISNULL(e.DeptID, 0) AS DeptID, ISNULL(d.Dept_Name, '') AS Dept_Name, ISNULL(e.SubDeptId, 0) AS SubDeptID 
                    FROM Employee e 
                    LEFT JOIN BasicDataInfo.dbo.Department d ON e.DeptID = d.Dept_ID 
                    WHERE e.EmpID = " + empId;
            ...
            try
            {
                string query2 = @"
                    ...
                    LEFT JOIN BasicDataInfo.dbo.Department d ON e.DeptID = d.Dept_ID 
                    WHERE e.EmpID = " + empId;
                DataTable dt2 = GetBasicDataTableData(query2);
                if (dt2 != null && dt2.Rows.Count > 0) return dt2;
            }
            catch { }
    ...
    public static bool HasEmployeeComplaintAuthority(decimal empId)
    {
        using (var con = GetConnection())
        using (var cmd = new SqlCommand("SELECT COUNT(*) FROM dbo.EmployeeComplaintAuthority WHERE EmpID = @EmpID AND IsActive = 1", con))
        {
    ...
    public static DataTable GetAuthorizedSubDepartments(int deptId, decimal? viewingEmpId)
    {
        if (!viewingEmpId.HasValue || viewingEmpId.Value <= 0)
        {
            return GetSubDepartments(deptId);
        }
        ...
        {
            string sql = @"
                SELECT DISTINCT sd.SubDept_Id, sd.SubDept_Name 
                FROM BasicDataInfo.dbo.SubDepartment sd
                INNER JOIN dbo.EmployeeComplaintAuthority eca ON sd.SubDept_Id = eca.SubDeptID
                WHERE sd.Dept_Id = @DeptID AND eca.EmpID = @ViewingEmpID AND eca.IsActive = 1
                ORDER BY sd.SubDept_Name";
    ...
    private static SqlParameter[] GetComplaintParams(int? deptID, int? subDeptID, string recordType, string status, DateTime? fromDate, DateTime? toDate, decimal? viewingEmpID)
    {
        ...
        if (viewingEmpID.HasValue && viewingEmpID.Value > 0)
        {
            pList.Add(new SqlParameter("@ViewingEmpID", viewingEmpID.Value));
        }
        return pList.ToArray();
    }

    public static DataTable GetComplaints(int? deptID, int? subDeptID, string recordType, string status, DateTime? fromDate, DateTime? toDate, decimal? viewingEmpID = null)
    {
        try
        {
            return ExecuteReader("dbo.sp_GetFeedbackAndComplaints", GetComplaintParams(deptID, subDeptID, recordType, status, fromDate, toDate, viewingEmpID));
        }
        catch (SqlException ex)
        {
            if (ex.Number == 8144 || (ex.Message != null && ex.Message.IndexOf("too many arguments", StringComparison.OrdinalIgnoreCase) >= 0))
            {
                return ExecuteReader("dbo.sp_GetFeedbackAndComplaints", GetComplaintParams(deptID, subDeptID, recordType, status, fromDate, toDate, null));
            }
            throw;
        }
    }
